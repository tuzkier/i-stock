...
3. 模式检索优先：在实现代码库（不是设计文档）按 surface 关键词（外部系统名、协议、adapter / client 名）用 Grep / Glob 检索同类既有实现；命中则引用具体文件路径作为复用证据，复用项目既有 API client、adapter、queue consumer、mock server、fixture 和 contract test 模式；未命中则记录检索范围与结论后才允许新建。
4. 实现前先补 contract / integration 失败路径测试；无法真实调用时使用项目认可的 stub、mock 或 sandbox。
5. 实现集成逻辑时隔离外部系统细节，避免泄漏到业务核心——以**可判定信号**约束：业务核心模块的 import / 类型引用不得出现外部 client SDK 类型或 transport 层错误码；外部响应必须先经 adapter 映射成领域类型后才允许跨边界。实现时列出「边界映射点」（哪个 adapter 在哪个位置把外部类型 / 错误码映射成领域类型）作为 contract 证据。
