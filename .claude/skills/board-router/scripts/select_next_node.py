...
def lane_rank(actions: dict[str, dict[str, Any]], lane: str) -> int:
    lanes = list(actions)
    try:
...
def stage_rank(config: dict[str, Any], lane: str, stage: str) -> int:
    work_graph = config.get("work_graph") if isinstance(config.get("work_graph"), dict) else {}
    ...
    stages = [str(item.get("stage") or "") for item in lane_config.get("stages") or [] if isinstance(item, dict)]
    try:
...
def execution_batch_key(node: dict[str, Any]) -> tuple[str, str]:
...
def parent_task_order_rank(node_id: str, node: dict[str, Any]) -> tuple[int, str, str]:
    order = node.get("execution_order")
    if isinstance(order, int):
        return order, str(node.get("parent_task_id") or ""), node_id
    if isinstance(order, str) and order.strip().isdigit():
        return int(order.strip()), str(node.get("parent_task_id") or ""), node_id
    parent_task_id = str(node.get("parent_task_id") or "")
    match = re.search(r"\bPT[-_]?(\d+)\b", parent_task_id, flags=re.IGNORECASE)
    if match:
        return int(match.group(1)), parent_task_id, node_id
    return 999_999, parent_task_id, node_id


def execution_batch_primary_nodes(
    selected_id: str,
    ...
    key_type, key_value = execution_batch_key(selected_node)
    if not (key_type and key_value):
        return [selected_id]
    selected_lane, selected_stage, _selected_action = lane_stage_for_node(config, selected_node)
    if not (selected_lane and selected_stage):
        return [selected_id]
    status_order = as_str_list(strategy.get("status_order")) or ["ready", "active"]
    status_set = set(status_order)
    accepts_kinds = as_str_list(action.get("accepts_kinds"))
    cohort: list[tuple[tuple[int, str, str], str]] = []
    for node_id, node in nodes.items():
        if execution_batch_key(node) != (key_type, key_value):
            continue
        if str(node.get("status") or "") not in status_set:
            continue
        node_lane, node_stage, _node_action = lane_stage_for_node(config, node)
        if node_lane != selected_lane or node_stage != selected_stage:
            continue
        node_kind = str(node.get("kind") or "")
        if accepts_kinds and node_kind not in accepts_kinds:
            continue
        cohort.append((parent_task_order_rank(node_id, node), node_id))
    ordered = [node_id for _rank, node_id in sorted(cohort, key=lambda item: item[0])]
    return ordered or [selected_id]
    ...
    for lane in schedulable_lanes(actions):
        for node_id in lanes.get(lane, []) or []:
            ...
            sort_key = (
                ...
                lane_rank(actions, lane),
                stage_rank(config, lane, str(node.get("stage") or "")),
                status_rank[status],
...
def mission_slice(
    ...
    primary_node_payloads: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    findings: list[Finding] = []
    ...
    primary_node_ids = primary_node_ids or [node_id]
    primary_node_payloads = primary_node_payloads or [node]
    directly_related: set[str] = set()
    for primary_node in primary_node_payloads:
        directly_related.update(as_str_list(primary_node.get("inputs")) + as_str_list(primary_node.get("outputs")))
...
def run(
    ...
    if not findings:
        ...
        if not selected_id or selected_node is None:
            ...
            _lane, _stage, selected_action = lane_stage_for_node(config, selected_node, findings)
            primary_node_ids = execution_batch_primary_nodes(
                selected_id,
                selected_node,
                nodes,
                selected_action,
            ...
            primary_node_payloads = [nodes[node_id] for node_id in primary_node_ids if node_id in nodes]
            expanded_context["primary_nodes"] = sorted(set(expanded_context["primary_nodes"] + primary_node_ids))
            expanded_related = set(expanded_context["related_nodes"])
            for primary_node in primary_node_payloads:
                expanded_related.update(as_str_list(primary_node.get("inputs")) + as_str_list(primary_node.get("outputs")))
            expanded_context["related_nodes"] = sorted(expanded_related)
            candidates = related_candidates(nodes, selected_id, expanded_context)
            slice_payload = mission_slice(
                mission_id,
                selected_id,
                selected_node,
                config,
                ...
                primary_node_payloads=primary_node_payloads,
            )
            if write_slice:
