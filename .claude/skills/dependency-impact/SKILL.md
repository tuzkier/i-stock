...
description: '当既有项目变更涉及现有模块/服务/API、外部依赖、共享服务、数据模型、跨团队接口，或用户询问影响范围/依赖关系时必须使用。当依赖是否存在只能靠猜测时立即使用，禁止靠想象输出依赖列表。触发词：依赖评估、影响分析、blast radius、依赖不确定、会不会影响到X。'
---
...
> 每条依赖必须有证据来源（文件路径、API 文档、配置文件、Graphify 查询结果）。
> 没有证据 → **先查对方仓库的 Graphify 索引 / YAPI 接口文档** → 仍无结果才标注 `[ASSUMED]` + 必须有验证行动项。
> "这个功能可能依赖 X" 是缺陷，不是分析结论。
> 禁止使用：可能、大概、应该、一般来说、通常。
> **证据获取顺序**：项目文档 → YAPI（query-api-docs）→ Graphify（对方仓库索引：graphify-exploring / graphify-impact-analysis）→ 标记 ASSUMED。
...
**第三层：自身代码**（前两层无阻塞时才执行）
- 自身代码哪些地方需要改？
- 改了之后谁会受到影响（下游调用方、数据消费方）？— 优先用 **Graphify** 做图索引分析（`graphify-impact-analysis`），得到 d=1/d=2/d=3 精确影响链
- 是否有破坏性变更？
...
| `graphify-exploring` | 第二层 + 第三层 | **多仓库均适用**：探索任意已索引仓库的模块结构和执行流。第二层用于理解外部系统代码，判断对方是否有能力提供接口/改动逻辑；第三层用于了解自身代码库全貌 |
| `graphify-impact-analysis` | 第二层 + 第三层 | **多仓库均适用**：分析任意已索引仓库中变更的 blast radius（d=1 直接炸 / d=2 很可能影响 / d=3 传递影响）。第二层用于评估让对方改动某处逻辑的风险；第三层用于分析自身代码的影响面 |

> **多仓库场景**：当本次任务需要**另一个团队的仓库**提供接口或修改逻辑时，若该仓库已在 Graphify 中建立索引，则在第二层（外部系统确认）就可以用 Graphify 分析对方代码——理解他们的结构、评估他们改动的可行性和风险，而不必等到第三层。

**证据优先级**：
- 第二层（外部系统）：`query-api-docs`（YAPI）> `graphify-exploring/impact-analysis`（对方仓库索引）> 标记 ASSUMED
- 第三层（自身代码）：`graphify-impact-analysis`（图索引） > 手动代码搜索 > 标记 ASSUMED

---

## 输出产物

产出 `harness-runtime/harness/artifacts/<mission-id>/discovery/dependency-impact.md`，内容见workflow.md。该文件默认是当前 Mission Slice 的 evidence artifact，不是独立调度 stage；只有当目标项目在 `work_graph.lanes` 显式注册 dependency-impact action 时，才按该 lane action 作为独立 Work Graph action 执行。
...
**执行约定：** 由本阶段主流程通过 `spawn_agent` 直接调用上述子 Agent。主流程必须先读取 `.harness/common/agents/<role>.md`，并把文件完整原文作为子 Agent prompt 第一段，再附加路径化 Task Envelope（角色目标、输入路径、输出路径、完成条件、write_scope / read_scope、同 barrier 角色关系和停止条件）。Task Envelope 只传路径和边界，不复述 role package 的角色边界，不粘贴上游文档全文，除非角色包或本 workflow 明确要求内联。等待返回时不得把固定短等待当作失败；返回后由主流程汇总裁决，并把结构化执行/审查结果登记到调用方 contract 的 `control_contract.dependency_impact` 与 `role_verdicts[]`；不得把子 Agent 调用替换为主 Agent 自审，也不得把结构化 YAML 追加到 `dependency-impact.md`。若 dependency-impact 被 `work_graph.lanes` 注册为独立 action，结构化结果由调用方写入该 action 配置指定的外部 contract / evidence artifact。

**结构化 claims 契约：** 调用方 contract 的 `control_contract.dependency_impact` 是 Gate 可检查事实面，`dependency-impact.md` 只是人读摘要。`required=true` 时至少包含：

```yaml
dependency_impact:
  required: true
  artifact: harness-runtime/harness/artifacts/<mission-id>/discovery/dependency-impact.md
  ...
  claims:
    - id: DEP-01
      ...
      source_evidence:
        - "<路径或查询证据>"
      failure_mode: "<若断言错误会造成什么失败>"
      validation_action: "<后续阶段如何验证或阻断>"
```

缺少 `claims[]`、缺少 claim 必填字段、缺少 `dependency-validity-reviewer` PASS，均视为调用方阶段 Gate 失败。
...
7. 确认 `graphify-impact-analysis` 技能是否可用（存在 `.harness/common/skills/graphify/graphify-impact-analysis/`）；若可用，读取 ``harness graphify status` + `graphify-out/GRAPH_REPORT.md`` 检查索引是否最新（若 "Index is stale" → 提示用户运行 `graphify .`）。对外部系统（另一个团队的仓库）证据，若该仓库已建 Graphify 索引，用 graphify-exploring / graphify-impact-analysis 在第二层直接取证。
8. 读取 dependency-impact role policy；默认执行子 Agent 为 `integration-impact-expert`，审查子 Agent 为 `dependency-validity-reviewer`；后续实际调度必须用 `spawn_agent` 调用。
...
<step n="2" goal="专业角色调度">
 - 用 `spawn_agent` 调用 `integration-impact-expert` 子 Agent；提供 Mission Contract、project-context、API 文档、配置、Graphify evidence；dependency claims 必须带 source evidence。
 - 分析完成后，用 `spawn_agent` 调用 `dependency-validity-reviewer` 子 Agent；提供 dependency-impact artifact、source evidence 和 confidence labels；无来源依赖声明必须 HOLD。结构化 verdict 必须用 `harness contract record-review` 写入调用方 contract 的 `role_verdicts[]`，并以 `--review-basis <dependency-impact.md>` 绑定审查对象；独立 action 场景写入该 action 配置指定的外部 contract / evidence artifact。`dependency-impact.md` 只保留面向人的审查摘要。
</step>
 ...
 **证据获取优先级（按顺序执行，前一步有结果可减少后续查询）**：
 ...
 3. **Graphify 对方仓库分析**：若对方系统的仓库已在 Graphify 建立索引，用 `graphify-exploring` + `graphify-impact-analysis` 直接分析其代码（见下方）
 4. **标记 UNCERTAIN 并向用户求证**：仅当以上全部无结果时

 > **反幻觉强化**：在标记任何外部依赖为 `[ASSUMED]` 或 `uncertain` 之前，
 > **必须先查 YAPI 接口文档；对方仓库已在 Graphify 中索引时再用 Graphify 分析其代码**。
 > 两者均无果才允许标记。

 **多仓库场景：用 Graphify 分析外部系统代码**

 当本次任务需要**另一个团队的仓库提供接口或修改逻辑**时（不是你自己的代码），若该仓库已在 Graphify 索引：

 ```
 // 4a. 探索对方仓库，理解其结构和能力
 READ graphify://repo/{对方仓库名}/context → 代码库全貌
 graphify_query({query: "<你需要的功能>"}) → 找到相关模块和执行流

 // 4b. 确认对方是否已有你需要的接口/逻辑
 graphify_context({name: "<对方的接口/函数名>"}) → 查看该接口的调用关系和实现位置
 ...
 **两种典型场景**：
 ...
 | 场景 | 用什么 | 目的 |
 |------|--------|------|
 | 对方已有接口但我不确定是否符合需求 | `graphify-exploring` | 直接查看接口签名、参数、返回值 |
 | 需要对方新增/修改逻辑 | `graphify-impact-analysis` | 评估他们的改动风险，给协作沟通提供依据 |
 ...
 |------|-----------|---------|---------|---------|------|
 | 通知中心 | POST /notify 发推送 | confirmed | uncertain | project-context.md 有提及，但无接口文档；Graphify 索引中找到 NotificationService，但无 /notify 端点 | 需对方新增接口，blast radius 评估：影响 d=1 两处 |
 | 用户服务 | GET /users/{id} 获取邮箱 | confirmed | confirmed | docs/user-service-api.md + yapi: #1234 | 无 |
 | 财务系统 | 对账数据同步 | uncertain | missing | 无任何文档，Graphify 未索引 | 需确认系统归属团队 |
 ```

 > **证据来源标注**：YAPI 标注 `yapi: <接口ID>`；Graphify 标注 `graphify: <仓库名>/<查询>`；项目文档标注文件路径。

 - 条件：项目文档和 YAPI 均无结果
 - 若对方仓库已在 Graphify 索引：用 `graphify-exploring` 探索其代码结构，确认接口是否存在
 - Graphify 有结果 → 整理为证据，更新状态（附对应来源标注）
 - 仍无结果 → 此时才允许标记为 UNCERTAIN/MISSING

 - 条件：需要对方系统新增或修改逻辑
 - 若对方仓库已在 Graphify 索引：执行 `graphify-impact-analysis`，评估对方改动的 blast radius
 ...
 **4.1 代码库全貌（使用 graphify-exploring）**：

 若 `graphify-exploring` 可用，先用 Graphify 了解代码结构，避免在陌生代码库中凭猜测定位变更范围：
 ```
 READ `harness graphify status` + `graphify-out/GRAPH_REPORT.md` → 代码库概览（模块数、staleness 状态）
 READ `mcp__graphify__query_graph`（社区） → 各功能区域及内聚性
 graphify_query({query: "<变更涉及的功能>"}) → 找到相关执行流和模块
 ```
 若索引过旧（"Index is stale"）→ 提示用户运行 `graphify .` 后继续。

 **4.2 变更范围定位**（必须实际扫描，禁止猜测）：
 - 结合 Graphify clusters/query 结果 + `project-context.md`，识别需要新增/修改的文件
 - 找到实际文件路径，确认现有代码中是否已有可复用逻辑

 **4.3 下游影响扫描（优先使用 graphify-impact-analysis）**：

 对每个将要修改的关键函数/接口/数据模型，使用 Graphify 进行 blast radius 分析：
 ```
 graphify_impact({
 target: "<函数名或接口名>",
 ...
 ```
 graphify_detect_changes({scope: "staged"}) → 映射当前改动到受影响的执行流
 ```

 若 Graphify 不可用，回退到手动代码搜索：
 ...
 **识别破坏性变更**：修改别人在用的接口签名、删字段、改枚举值等。
 ...
 | 模块/文件 | 变更类型 | 定位来源 |
 |---------|---------|---------|
 | src/services/order.ts | 新增状态变更逻辑 | graphify_query("order status") → OrderService |
 | src/jobs/reconcile.ts | 新增，需要创建 | 无现有文件 |

 ### 下游影响（Graphify blast radius）
 ...
 > 证据来源：graphify_impact({target: "OrderService.updateStatus", direction: "upstream"})
 ```
</step>
...
<step n="8" goal="依赖有效性审查（审查-修复循环）">
 ...
 唯一合法退出条件：dependency-validity-reviewer 在等同严格度下返回 PASS / 无阻断。轮次只记录修复历史，永不构成放行理由。 -->
 - 循环：无轮次放行（producer-fixable 缺口不设通过上限，轮次只记录修复历史）；退出条件：本轮 dependency-validity-reviewer 在等同严格度下返回 PASS / 无阻断
 ...
 - 条件：卡死——同一阻断性发现在修复后审查员仍以相同根因连续 HOLD 且无实质进展（按缺口本质判断，不是"轮次到点"）
 - 不得降级通过。按 `core.md`「严格审查不变量」重新归因：producer 能补则留在循环升级修复策略继续重做；本质是依赖事实真实未知（需查证 / 需对方配合）则发起 Decision Gate 让用户裁决方向。
 - 仅当确需用户拍板才能解时，发起 Decision Gate，向用户展示遗留阻断问题清单，等待用户回复；候选不含"接受遗留 / 接受降级 / 降级通过 / 重置轮次继续"，只含：给出修复方向继续修 / 改范围或回流上游 / 升级 BLOCKED
 - 条件：用户提供了解决方向或授权调整
 - 按用户指导修改 `dependency-impact.md`，重新回到 round_start 继续审查循环
 - 残留风险只能由用户在充分披露遗留阻断后于 Decision Gate 显式拥有：仅此时调用 `harness-cli` 执行 `harness approval append --mission <mission-id> --type tradeoff --stage dependency-impact --status approved --comment "<用户原话>" --json`（不得只在文档末尾 prose 记录），当前 Mission Slice dependency evidence 或独立 action 的外部 contract 必须保留完整未解决 findings 并标注 `accepted_by_user=true`。审查循环本身永不把未解决阻断自动转为通过。
...
| "我改的这个函数应该没有其他地方调用" | 用 graphify_impact 查一下再说，d=1 的直接调用方往往超出预期 |
| "这个字段改动影响不大" | graphify_detect_changes 跑一遍，看看有多少执行流经过这里 |
