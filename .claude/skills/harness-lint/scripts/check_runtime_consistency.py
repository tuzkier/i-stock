...
WORKFLOW_EXPECTATIONS = {
    ...
    ".harness/common/skills/interaction/workflow.md": {
        "must_reference_path": [
            ...
            "harness interaction prototype-check",
            "harness interaction project",
            "harness interaction gate run",
            "harness alignment check",
        ],
        "must_mention_concept": ["interaction-spec", "behavior-graph", "prototype-check"],
    },
    ...
    ".harness/common/skills/project-lint/workflow.md": {
        ...
        "must_mention_concept": ["command evidence", "trace 检查", "project-lint-report.json", "gate_effect"],
    },
...
CLI_COMMAND_EXPECTATIONS = [
    ...
    ["interaction", "prototype-check"],
    ["interaction", "project"],
    ["interaction", "visual-coverage-check"],
    ["interaction", "trace-coverage-check"],
    ["interaction", "resolve-feedback"],
    ...
    for routing_root in routing_roots:
        ...
        for pattern_glob in ("*.md", "*.html"):
            for path in sorted(routing_root.rglob(pattern_glob)):
                if path.match("*/harness-lint/SKILL.md"):
                    continue
                text = path.read_text(encoding="utf-8")
                for pattern in FORBIDDEN_WORK_GRAPH_ROUTING_PATTERNS:
                    if pattern in text:
