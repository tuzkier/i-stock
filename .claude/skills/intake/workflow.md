...
- 升级到 brainstorm：若轻对话发现需求不只是"要不要启动"的问题，而是"具体要做成什么样还没想清"、需要反复发散 / 研究才能定下"要什么"，或用户显式说"先脑暴 / 先讨论想透 / 帮我把需求想清楚 / 先别急着开干"，则退出本阶段、改走 `brainstorm`。brainstorm 在接入前把"要什么"想透并落成脑暴活文档，收敛后再交棒本阶段。不要在 intake 轻对话态里自己硬塞一个深度脑暴。
...
`source_material` 落盘（资料稳定化，不进任务目标）：用户提供的文档 / 链接 / 截图 / 讨论材料属于完备性文档集第二类“人提供资料”，必须落到项目根 `materials/` 目录，使下游审查员能稳定纳入文档集。
- 可拷入的文件型资料（上传文档、截图、粘贴文本另存）直接拷贝进 `materials/`；外部目录 / 链接 / 在线文档不拷文件，登记到 `materials/_sources.md`（记标题、来源地址 / 路径、本 mission 引用原因）。
- `materials/` 不带 mission id、长期跨 mission 累积；同名资料已存在且内容一致时不重复拷入。
- 把本次实际引用到的 `materials/` 下文档路径收进 `raw_intake_brief.source_materials`，作为后续写入任务契约 `source_materials` 引用清单的依据（只记引用了哪些文档，非自由文本）。
- 防污染语义不变：`source_material` 仍不写入任务目标、工作图节点标题或 `success_definition.deliverables`（见硬关口 `meta-instruction-not-objective`）；落盘只给它一个稳定位置，不改变它的非目标定位。

脑暴收敛物接入（由 `brainstorm` 交棒而来时）：
- 存在**已收敛**脑暴活文档 `harness-runtime/harness/brainstorms/<slug>.md` 时，把它路径收进 `raw_intake_brief.source_materials` 与 `discussion_outputs`（整体作为讨论产物，默认不进目标）。
- 脑暴活文档中**用户已确认的收敛"目标"**写入 `raw_intake_brief.actual_task_goal` 候选，"用户确认原话"写入 `execution_confirmation`；二者仍按本阶段后续 `task_goal_fidelity` 与执行意图关口校验，不因来自脑暴而免检。
- 脑暴的"范围方向""验收轮廓"作为构造 `objective` 摘要、范围与待探索问题的**参考输入**，不直接当成最终范围、验收场景 / 条件或业务对象结论。
...
`git-workflow prepare` 之前不得写 `mission-status.yaml` / 工作图节点 / 任务切片（Mission Slice）/ 关口报告（Gate report）。
...
记录为种子节点。
- 条件：是新需求
调 `harness graph node create --node-id <id> --kind <kind> --title <title> --lane <lane> --stage <stage> --status <status> --mission-id <mission_id> --json`。kind 来自 `harness config snapshot` 的 `work_graph.node_kinds`：需求 → `REQ-*` / `EPIC-*`；缺陷 → `BUG-*`；调研 → `RESEARCH-*`。创建后再调 `harness graph rebuild` + `harness graph check`。

调 board-router 为 `mission_id` 创建 / 恢复任务切片（Mission Slice）：`harness mission create-slice` 或 `harness board select --write-slice`。
...
通过 `Task(subagent_type="mission-framing-expert", prompt=<Task Envelope>)` 工具调用 `mission-framing-expert` subagent
...
通过 `Task(subagent_type="mission-contract-effectiveness-reviewer", prompt=<Task Envelope>)` 工具调用 `mission-contract-effectiveness-reviewer` subagent
...
按缺陷性质分流，不得一律由编排者 inline 修补任务契约：

  - **实质缺陷回产出者重做**：当保持（HOLD）/ 阻断（BLOCKED）涉及推理链断裂（`reasoning_chain_open`）、目标污染（交付物 / 验收条件 / 工作图标题混入非真实任务目标语料）、用户故事产品上下文缺失（`story` 缺用户 / 问题 / 场景 / 成功指标）、或治理判定断链（治理风险结论与 `autonomy_level` / 检查点 / 风险维度之间缺可追溯依据）这类完备 / 自洽缺陷时，回第 2 阶段重新 `通过 `Task(subagent_type="mission-framing-expert", prompt=<Task Envelope>)` 工具调用 `mission-framing-expert` subagent` 重做 `intent-framing.yaml`（任务信封须携带本轮 `role_verdict` 的对应 findings 与缺陷分类），再依次经第 3 / 4 阶段重建治理判定与任务契约（Mission Contract），不得由编排者直接改写 `mission-contract.md` 绕过产出者。
  - **ID 对齐 / 格式类才允许 inline**：仅当缺陷限于稳定 ID 对齐、字段格式、追溯锚点拼写、模板段落顺序等不触及意图 / 目标 / 范围 / 治理判定的表层问题时，编排者可直接修复 `mission-contract.md` 与 `contract.yaml`。

调 `harness trace step-exit --mission <mission_id> --step phase-5-review --rounds <n> --status fail --json`，回循环开头重审全文（回第 2 阶段重做的，重建后回第 5 阶段审查员循环开头）。
...
- 条件：卡死——同一阻断在回第 2 阶段重做后，审查员仍以相同根因连续 HOLD 且无实质进展（按缺口本质判断，不是"轮次到点"）
不得降级通过。按 `core.md`「严格审查不变量」重新归因：producer 能补则留在循环升级修复策略继续重做；本质是任务范围 / 验收口径需用户定义才能解时，调 AskUserQuestion，候选（**不含"接受降级 / 降级通过"**）：

- **A：调整范围 / 验收条件** → 按用户指导修改 mission-contract.md，回循环开头
...
审查摘要附加到 `mission-contract.md` 末尾；结构化 `role_verdicts` 通过 `harness contract add-verdict` 写入 `contract.yaml`。
...
| 审查员最大轮次用尽 | 第 5 阶段 | AskUserQuestion 三选项（调整 / 降级 / 决策关口） |
