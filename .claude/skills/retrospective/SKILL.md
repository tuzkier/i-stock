...
retrospective 对应 RUP 项目管理中的迭代评估和环境工作流中的流程改进。它不改变已交付范围，而是把本次需求实现链路中的计划偏差、失败模式、流程缺口、知识缺口和可落地改进项提炼出来，反馈到规则、模板、工作流、检查器、测试、Agent prompt 或 project-knowledge。

## 阶段能力

复盘必须回答：本次交付链路有哪些可复用经验、哪些系统性缺口需要修正、哪些知识需要沉淀、哪些交付真实性问题必须回流处理。

复盘产物不能停留在“做得好 / 做得不好 / 下次注意”。每条改进都必须有明确目标位置、证据来源、执行条件和后续归宿；如果发现交付证据、验收路径或风险披露不足以支撑 delivered 结论，必须触发重新验证或 course-correction，而不是在复盘里悄悄接受。
...
| 步骤 | 产出 |
|------|------|
| 收集交付包、验收结果、阶段门报告、审查 / 验证发现和工作图历史 | 事实基础 |
| 分析计划偏差、跨阶段失败模式和交付真实性回查 | 复盘判断 |
| 提取指向规则、模板、工作流、检查器、测试、Agent prompt 或项目知识的改进 | 可执行改进 |
| 更新 project-knowledge / project-context.md | 持久化经验 |
...
<step id="step-2" n="2" goal="dispatch 规划分析 Agent">
 通过 `Task(subagent_type="planning-analyst", prompt=<Task Envelope>)` 工具调用 `planning-analyst` subagent
...
<step id="step-4" n="4" goal="更新 project-context.md">
 ...
 - 由主 Agent 基于候选计划、沉淀账本和阶段产物继续做语义提炼，把稳定产品知识、行为规格、设计决策、工程样板、运行手册或教训写入 `project-knowledge/<domain>/...`。interaction 阶段通过审查的 prototype project patch、系统 surface、原型模式、界面信息架构、领域对象到界面的映射、可复用交互约束和 interaction-spec consistency decision 必须作为候选评估；主可操作原型本体不进 `project-knowledge/`（随分支合并留在独立原型工程目录），稳定 surface 细节同步到 `project-knowledge/product/ui-surfaces/`、**稳定区域树 / 布局骨架随 `harness knowledge promote` 并入项目级累积图 `product/system-use-cases/behavior-graph.yaml#regions`**、**本次确认的可复用设计 token / 基础组件 / 业务组件（绑 OBJ/SUC）/ 布局与交互约定增量并入 `project-knowledge/product/design-system/` 对应分层文件（业务组件→`business-components.md`、基础组件→`base-components.md`、token→`design-spec.md` 名注册 + `prototype/tokens.css` 真值、原则/框架→对应文件）；只沉淀可复用、不沉淀一次性样式；npm 包库原生组件不进 design-system 增量（它住 `base-components.md` 的库登记段、由库维护），只沉淀项目自研 / 二次封装组件与项目主题定制 token；每条保留 source mission / status / confidence，绝不整份覆盖**，若它们确立或改变用户可观察行为，且 `spec.enabled=true`，必须同步评估是否进入 `project-knowledge/specs/<capability>/spec.md`。不得整份复制 stage artifact；每条新增/修改知识必须保留 source mission、status、confidence。
   - **design-system 增量一律先进候选、人审 approve 才落 canonical（强制，新增 / 变更 / 冲突 / 删除一律）**：把本次所有 design-system 候选条目先写入候选暂存区 `harness-runtime/harness/artifacts/<mission-id>/retrospective/design-system-candidates.md`，**不直接写 `project-knowledge/product/design-system/**` 与 `prototype/`**；每条标动作类型（add / change / conflict / remove）+ 对 canonical 的 `before/after diff` + source mission + 理由。候选清单生成 Decision Gate 交用户逐条裁决（可批量 approve 同类）：**approve 才并入对应分层文件 + `prototype/`（add→追加、change→按确认值更新、一致→升 confidence、conflict→按裁决取值）；reject 则丢弃候选、canonical 零改动，并在 `knowledge-promotions/<mission-id>.md` 记「已评估·拒绝·原因」**——被拒条目即废弃，不留半截写入、不反向手删。无用户裁决前不得静默并入 canonical。
