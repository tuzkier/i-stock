...
5. **实现真实行为**：先做**模式检索**——在实现代码库（不是设计文档）按 surface 关键词（handler / service / repository 名、领域名词、错误码）用 Grep / Glob 检索同类既有实现；命中则引用具体文件路径作为复用证据，复用既有 controller、service、repository、validator、error mapper、transaction helper 和测试 fixture，不为测试通过绕过真实调用链；未命中则记录检索范围（关键词 + 路径）与「无同类实现」结论，之后才允许新建。
6. **错误路径与兼容性**：错误路径覆盖不靠记忆清单，靠**导出规程**——按「每个入参的非法/缺失/越界值 × 资源状态（不存在 / 已删 / 无权 / 冲突） × 下游（超时 / 失败 / 部分成功）」做笛卡尔枚举，再筛掉本任务不适用项；把「筛掉的项 + 理由」一并作为证据列出，使覆盖性可被审查逐项复核而非凭感觉。兼容性窗口（backward compatibility）按同样方式枚举旧调用方契约。
7. **数据一致性**：所有写入必须说明事务边界、失败回滚、幂等或重复提交行为，并以 **fault-injection 取证规程**证明（作为 writes 场景 required evidence）——幂等：同一请求体连发 2 次，断言副作用只发生一次；事务回滚：在写入链路中段注入异常，断言无残留写入；并发：并行触发冲突，断言最终态唯一。涉及 schema / migration / backfill 时停止并要求 `data-engineer` 或 Decision Gate。
...
- transaction / idempotency / concurrency evidence when writes, state transitions or retries are involved（按 Expert Method 第 7 步 fault-injection 规程取证：幂等连发去重、链路中段异常无残留、并发冲突终态唯一）。
