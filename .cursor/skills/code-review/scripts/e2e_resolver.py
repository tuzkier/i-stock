...
FRONTEND_PACKAGE_RELS = [
    "package.json",
    "apps/frontend/package.json",
    "frontend/package.json",
    "web/package.json",
    "TheForce/web/package.json",
]
...
FRONTEND_CONFIG_PATTERNS = [
    ...
    "TheForce/web/playwright.config.*",
]

FRONTEND_ARTIFACT_BASES = ["", "apps/frontend", "frontend", "web", "TheForce/web"]
...
def package_manager(root: Path, package_dir: Path | None = None) -> str:
    base = package_dir or root
    ...
    if package_dir is not None:
        return package_manager(root)
    return "npm"


def install_command(root: Path, packages: list[str], package_dir: Path | None = None) -> str:
    ...
    if package_dir is not None and package_dir != root:
        ...
        if manager == "pnpm":
            ...
            prefix = f"--prefix {rel} "
    if manager == "pnpm":
        return f"pnpm {prefix}add -D " + " ".join(packages)
    if manager == "yarn":
        return f"yarn {prefix}add -D " + " ".join(packages)
    return f"npm {prefix}install -D " + " ".join(packages)
...
def package_command(root: Path, script: str) -> str | None:
    for rel in FRONTEND_PACKAGE_RELS:
        ...
        package_rel = package_dir.relative_to(root)
        if manager == "pnpm":
            return f"pnpm --dir {package_rel} run {script}"
        if manager == "yarn":
            return f"yarn --cwd {package_rel} {script}"
        return f"npm --prefix {package_rel} run {script}"
    return None


def existing_artifacts(root: Path) -> dict[str, Any]:
    html_candidates = []
    for base in FRONTEND_ARTIFACT_BASES:
        prefix = f"{base}/" if base else ""
        html_candidates.extend([f"{prefix}playwright-report/index.html", f"{prefix}cypress/reports/index.html"])
    traces: list[str] = []
    videos: list[str] = []
    screenshots: list[str] = []
    artifact_bases: list[str] = []
    for base in FRONTEND_ARTIFACT_BASES:
        prefix = f"{base}/" if base else ""
        artifact_bases.extend([f"{prefix}test-results", f"{prefix}cypress"])
...
def detect_e2e_toolchain(root: Path, config: dict[str, Any], mission_id: str | None) -> list[dict[str, Any]]:
    package_rels = [rel for rel in FRONTEND_PACKAGE_RELS if (root / rel).exists()]
    deps: set[str] = set()
    scripts: dict[str, str] = {}
    for rel in package_rels:
        deps.update(dependency_names(root, rel))
        scripts.update(package_scripts(root, rel))
    ...
    playwright_configured = (
        ...
        or file_exists_any(root, FRONTEND_CONFIG_PATTERNS)
    )
    cypress_configured = (
        "cypress" in deps
        or any("cypress" in script for script in scripts.values())
        or file_exists_any(root, [
            "cypress.config.*",
            "apps/frontend/cypress.config.*",
            "frontend/cypress.config.*",
            "web/cypress.config.*",
            "TheForce/web/cypress.config.*",
        ])
    )
...
def _frontend_flowstep_obligations(root: Path, mission_id: str) -> dict[str, Any] | None:
    """门B：读 interaction 阶段 prototype-as-frontend.contract.yaml 的
    ``e2e_obligation[]`` 与 PRD flow-step 全集，计算 flow-step 覆盖。返回
    ``{prd_flowsteps, coverage, covered_flowsteps, uncovered_flowsteps}``，
    contract 缺失时返回 None。纯解析（无副作用）。"""
    contract_path = (
        root / "harness-runtime/harness/stages" / mission_id / "contracts" / "prototype-as-frontend.contract.yaml"
    )
    if not contract_path.exists():
        return None
    try:
        # 脚本位于 <common>/skills/code-review/scripts/；parents[3] == <common>（含 harness_cli_core）。
    ...
    if not contract:
        # 兼容无 control_contract 包裹的扁平 contract。
        contract = load_yaml(contract_path)
    obligations = contract_e2e_obligations(contract if isinstance(contract, dict) else {})
    prd_flowsteps = prd_flowsteps_for_mission(root, mission_id)
    cov = frontend_flowstep_obligation_coverage(prd_flowsteps, obligations)
    return {
        "prd_flowsteps": sorted(prd_flowsteps),
        "e2e_obligation_count": len(obligations),
        **cov,
    }
...
def resolve(root: Path, mission_id: str | None) -> dict[str, Any]:
    ...
    has_package_json = any((root / rel).exists() for rel in FRONTEND_PACKAGE_RELS)
    ...
    # 门B（frontend_engineering 路线）：可达性下沉 E2E，以 flow-step 全集为键。
    # 仅在 delivery_mode==frontend_engineering 且存在 interaction 阶段 contract 时激活；
    # interactive / 普通后端项目默认分支不触发，保持现有 task-driven 行为不变。
    frontend_flowstep_obligations: dict[str, Any] | None = None
    delivery_mode = str(
        ((config.get("prototype") or {}).get("delivery_mode") if isinstance(config.get("prototype"), dict) else "")
        or ""
    ).strip()
    if delivery_mode == "frontend_engineering" and mission_id:
        frontend_flowstep_obligations = _frontend_flowstep_obligations(root, str(mission_id))
        if frontend_flowstep_obligations and frontend_flowstep_obligations.get("uncovered_flowsteps"):
            decision_gate_reasons.append({
                "reason": "frontend_flowstep_e2e_uncovered",
                "flow_steps": frontend_flowstep_obligations["uncovered_flowsteps"],
            })
    ...
    return {
        ...
        "frontend_flowstep_obligations": frontend_flowstep_obligations,
        "artifact_policy": {
