...
1b. **全局闭包校验(运行,非只读约定)**:本步必须**实际运行** `python3 COMMON_ROOT/skills/stage-gate/scripts/check_closure.py --root . --mission <mission-id> --json`,把全 mission 跨阶段 contract 的 `traces_to` 悬空引用扫一遍(对应不变量 `mission-closure-resolved`)。灰度期 status=WARN 不阻断交付,但悬空项必须在交付包的"已知缺口 / 风险"里如实披露;校准稳定后加 `--strict` 令悬空升 FAIL 阻断。命令输出即证据,不得跳过或口头声称已查。
...
| `mission-closure-resolved` | 全 mission 跨阶段闭包:所有阶段 contract 的 `traces_to` 引用的稳定 ID 在文档集内都有定义,无悬空引用(推理链不断在文档集外)。各阶段 reviewer 做局部完备,本门做全局一次性兜底 | script=stage-gate/scripts/check_closure.py --mission &lt;id&gt;(灰度期 WARN,校准后 --strict 升 FAIL) |
...
<step id="step-1" n="1" goal="专业角色调度">
 通过 `@release-readiness-expert` native delegation调用 `release-readiness-expert` subagent（Cursor auto-routes 到对应 agent registry 项）
 - 任务信封:验证证据路径、代码审查发现路径、验收场景 / 条件追踪、已知风险、审批记录、写入范围、完成条件和风险边界要求。
 通过 `@acceptance-package-reviewer` native delegation调用 `acceptance-package-reviewer` subagent（Cursor auto-routes 到对应 agent registry 项）
 - 交付物生成后调度审查员,任务信封包含:验收结果路径、交付包路径、验收证据索引、只读约束和结论输出格式。Markdown 只保留面向人的交付审查摘要。
 - 审查 - 修复 - 重审闭环(显式消费审查员暂停结论,无轮次放行——轮次只记录修复历史,永不构成放行理由,每轮以等同严格度重审):审查员返回暂停结论时,不得直接进入 handoff,必须按缺口 category 分流修复,修复后**重新调度 acceptance-package-reviewer 重审**,直到审查员在等同严格度下 PASS。
   - `reasoning_chain_open` / `internal_contradiction` 类:回 release-readiness-expert 重做对应章节(不由编排者就地补硬编理由),完成后重审。
   - `needs_user_clarification` 类:转 step-7 的 `AskUserQuestion` / `blocking-followup-not-accepted` 决策门取得用户裁定,回 release-readiness-expert 落章后重审。
   - 其余可由产出者直接补齐的链断(`acceptance_evidence_missing` / `not_handover_ready` / `not_self_contained` 等):回 release-readiness-expert 补齐对应章节后重审。
   - 卡死(同一 category 缺口在 release-readiness-expert 重做后仍以相同根因连续暂停且无实质进展,按缺口本质判断、不是"轮次到点"):不得降级通过,停在交付阶段,按失败路径发起决策门(Decision Gate)交用户裁定(候选不含"降级通过 / 接受遗留",残留风险只能由用户在充分披露后显式拥有并记 approval),不得 handoff 或把阶段置为完成。
</step>
...
<step id="step-4" n="4" goal="遗留项和后续">
 ...
 - 对每个遗留项给出:描述、来源、影响范围、用户后果、严重性(必须处理 / 建议处理 / 可忽略)和建议处理方式。严重性不得凭印象给,按下面的判定矩阵逐项命中后再定级。

 **严重性判定矩阵**(复用任务接入(intake)治理风险硬触发项口径):

 | 命中条件 | 严重性 | 含义 |
 |---|---|---|
 | 命中 安全 / 数据 / 权限 / 验收阻断 / 不可逆 任一(治理风险硬触发项口径) | **必须处理(阻断)** | 阻断本次交付,未被用户显式接受风险前不得算完成 |
 | 影响用户体验或正确性,但存在 workaround,用户可绕过继续使用 | **建议处理** | 不阻断交付,需在交付包披露后果与建议 |
 | 不影响验收、安全、数据、权限,无用户可感后果 | **可忽略** | 仅归档记录,不进披露主线 |

 - 定级规则:从上往下命中即取该级,**只要命中阻断条件就是「必须处理」,不允许因为"有计划后续修"而下调**。无 workaround 的体验 / 正确性问题就近按「必须处理」对待,不得归入「建议处理」。
...
<step id="step-6" n="6" goal="后续事项归类与移交">
 ...
 - 若存在必须后续处理的遗留项,交付文档必须按 step-4 的严重性判定矩阵把它归类为阻断性(必须处理)、建议性(建议处理)或可忽略,并说明用户影响和建议处理方式。
 - 凡按矩阵命中「必须处理(阻断)」的遗留项且用户未接受风险,不得把当前增量写成完成;应返回修复、补验证或发起用户决策。是否触发本条,直接由 step-4 矩阵的定级结果决定,不另设判据。
</step>
...
<step id="step-7" n="7" goal="最终验收 Checkpoint">
 ...
 - 条件:审查员返回 `needs_user_clarification` 类缺口(交付边界归属模糊、deferred / returned 归宿判定、残留风险是否可接受等真实信息缺失,不可由产出者硬编补齐)
  - 不逼 release-readiness-expert 硬编理由,转向用户澄清:用本步 `AskUserQuestion` 把每条 `needs_user_clarification` 缺口列成具体问题(缺口描述、涉及的验收场景 / 条件或章节、待用户决定的归属 / 归宿 / 风险接受),取得用户裁定后回 release-readiness-expert 据此补齐对应章节,再重新调度审查员重审。
  - 若缺口属于阻断性后续项是否被用户接受的判定,按失败路径 `blocking-followup-not-accepted` 发起决策门(Decision Gate),候选处理为创建后续工作、降级为建议项或停止任务;依用户决策结果回 release-readiness-expert 落章后重审。
</step>
...
<step id="step-8" n="8" goal="Artifact Gate 自检">
 ...
 - 调用命令行接口 `harness gate run` `--stage delivery --mission ${mission-id} --artifact harness-runtime/harness/stages/${mission-id}/delivery-package.md --json`,证据为必需。
 - 条件:gate status != PASS
  - 按缺口归属决定修复者:`reasoning_chain_open` / `internal_contradiction` 类(交付包结论推理链断裂、文档集内自相矛盾)回 release-readiness-expert 重做对应章节,不由编排者就地补,以免硬编理由凑满链条;仅结构 / 契约类 gate 失败(契约字段缺失、最小结构不完整、审批记录缺失等)才由编排者就地修复 delivery-package.md、acceptance-result.md、审批记录或 contract。修复后重新运行 gate;不得 handoff 或完成阶段。
 - 条件:结构不完整
  - 自行补充缺失部分,不要跳过。
</step>
...
<step id="step-9" n="9" goal="任务收尾 + handoff">
 ...
 - 条件:结论为 delivered(所有验收场景 / 条件通过且无阻断性遗留项,第 7 步已取得用户验收,第 8 步 Gate 已 PASS)
  - 调用命令行接口 `harness delivery handoff` `--mission ${mission-id} --pause`,证据为必需。
  - 调用命令行接口 `harness mission stage complete` `--mission ${mission-id} --stage delivery`,证据为必需。
...
| `gate-fail` | `harness gate run --stage delivery` 返回失败 | 交付内修复后重跑阶段门。 |
