...
<step n="8" goal="依赖有效性审查（审查-修复循环）">
 ...
 - 先按 reviewer 给出的 HOLD category 对每条阻断性发现分流，再决定由谁修复：
  - 实质推理缺陷（完备性断链：依赖链 / blast radius 漏判或断裂；自洽性冲突：三层结论或 claim 相互矛盾；无来源 claim：dependency 声明缺 source evidence）——不得由编排者就地改文。必须用 `spawn_agent` 重新调用 `integration-impact-expert`，brief 带上本条发现、reviewer 指出的缺口 / 矛盾、相关 source evidence 与 Mission Contract 约束，要求其基于证据重做对应条目（补链、消解矛盾、补充或标注来源），由产出者写回 `dependency-impact.md`。
  - 仅面向人的摘要排版 / 标注问题（行动清单措辞、Blocker 排版、待验证项标注、证据缺口表格格式等不改变推理结论的项）——编排者就地修订 `dependency-impact.md` 即可，无需回产出者。
 - 记录本轮每条发现的 category、分流去向（回 `integration-impact-expert` / 就地）与修复结果
 ...
 - 条件：卡死——同一阻断性发现在重新调用 `integration-impact-expert` 子 Agent 重做后，reviewer 仍以相同根因连续 HOLD 且无实质进展（按缺口本质判断，不是"轮次到点"）
 - 不得降级通过。按 `core.md`「严格审查不变量」重新归因：producer 能补则留在循环升级修复策略继续重做；本质是依赖事实真实未知（需查证 / 需对方配合）则发起 Decision Gate 让用户裁决方向。
 - 仅当确需用户拍板才能解时，发起 Decision Gate，向用户展示遗留阻断问题清单，等待用户回复
 - 条件：用户提供了解决方向或授权调整
