...
> **执行智能体（Agent）**：由本阶段 `workflow.md` 的角色策略（role policy）声明；若运行时无法调度所需智能体角色（Agent role），停在关口（Gate）并报告角色不可用，不得由主智能体（Agent）按本工作流自写自审。

<HARD-GATE>
复杂度 >= medium 的任务，在产品定义阶段（PRD）之前必须执行探索。
跳过探索直接写产品定义阶段（PRD）= 违规。
</HARD-GATE>
...
在写产品定义阶段（PRD）之前先探索技术可行性，避免“拍脑袋设计，写完发现不行”。

本阶段按统一过程（RUP）思想只做产品定义阶段（PRD）的前置取证：发现业务对象候选、业务活动、系统边界、责任划分和原型承载线索。每类线索都必须带识别方法、证据来源、约定模板和停止边界；不得只列名称，也不得提前产出最终业务对象模型、系统用例或页面方案。
...
<HARD-GATE>
当探索涉及既有项目、现有代码理解、项目/架构分析、调用链或执行流时，若 `.harness/common/skills/graphify/graphify-exploring/` 存在，必须先使用 `graphify-exploring` 建立代码库全貌，再进入方案比较。
</HARD-GATE>

执行要求：
- 读取 `.harness/common/skills/graphify/graphify-exploring/SKILL.md`
- 先跑 `harness graphify status --json`：
  - `cli_installed=false` ⇒ Graphify PyPI 包还没装到本机；引导用户跑 `uv tool install graphifyy`（或 `pipx install graphifyy` / `pip install graphifyy`，PyPI 名是 `graphifyy` 双 y，CLI 是 `graphify`）。安装完再继续。
  - `cli_installed=true && available=false` ⇒ 装了 CLI 但项目没索引；跑 `graphify .` 建立 `graphify-out/`，或在 `degradations[]` 显式记录 `graphify_unavailable` 并写补救动作。
  - `available=true && fresh=false` ⇒ 索引过期（>24h）；跑 `graphify --update` 或 `graphify .`，或在 `degradations[]` 记录 `graphify_stale`。
- 检查 Graphify 仓库上下文：``mcp__graphify__query_graph`` 与 ``harness graphify status` + `graphify-out/GRAPH_REPORT.md``
- 用 `mcp__graphify__query_graph` 查询本次任务相关概念、模块或执行流
- 对关键符号用 `mcp__graphify__get_node` 深入调用关系
- 如果索引缺失、过期或 MCP 工具不可用，记录降级原因，并给出补救动作（通常是 `graphify .`）
- Graphify 状态必须与 Harness CLI 状态分开汇报：`graphify.status` 的 WARN / 未索引只说明代码索引证据降级，不得表述为 Harness 控制面不可用、Mission 不存在或 CLI 不能用。
- 禁止在 Graphify 可用时只用 grep / 文件列表完成“项目分析”
...
- 变更范围 ≤ 3 个文件
- 不引入新模块或新的外部依赖
- 任务契约的范围和验收条件已经具体到可直接写产品定义阶段（PRD）的程度
- 用户没有要求探索
...
digraph discovery {
 ...
 decision [label="发起决策关口\n候选路线+权衡"];
 done [label="探索完成\n→ 产品定义" shape=doublecircle];
...
## 阶段要素模型（Stage Element Model）

本阶段必须维护的关键要素见 `.harness/docs/methodologies/stage-element-model.md#discovery`。摘要：

| 要素 | 使用方 | 缺失后的失败模式 |
|---|---|---|
| 既有能力（Existing Capability） | 产品定义（PRD）/ 方案 / 技术设计 | 重复设计或破坏既有行为 |
| 受影响能力（Affected Capability） | 产品定义（PRD）/ 依赖影响 / 审查 | 影响半径（blast radius）被低估 |
| 证据来源（Evidence Source） | 产品定义（PRD）/ 阶段关口（Stage Gate） | 结论不可追溯 |
| 假设（Assumption） | 产品定义（PRD）/ 方案 / 决策关口（Decision Gate） | 假设被当成事实 |
| 风险 / 约束（Risk / Constraint） | 方案 / 技术分析 | 下游设计绕过风险 |
...
<exit>
  ...
  - `gate-pass`: `harness gate transition --stage discovery` 返回 `status=pass`
</exit>
...
<step n="1" goal="专业角色调度">
 - 通过 `@discovery-analyst` native delegation调用 `discovery-analyst` subagent（Cursor auto-routes 到对应 agent registry 项），仅追加路径化任务信封；任务信封只包含：任务目标、输入路径（任务契约、`project-context` 如存在、`project-knowledge/specs` 索引如启用）、输出路径、`write_scope`、可用工具（Graphify 可用性只给状态和查询入口，不粘贴大段结果）和事实来源 / 置信度要求。角色提示由安装时物化的 `.<adapter>/agents/discovery-analyst.md` 加载，工作流（workflow）内不再复制原文，也不得复述或改写 discovery-analyst 的角色边界。
 ...
 **当前现状**
 ...
 - 记录降级原因，不得静默回退
 - 对用户和阶段产物说明时必须把 Graphify 降级与 Harness CLI / Mission 状态分开；Graphify 未索引只影响代码事实取证，不代表控制面不可用。
...
<step n="6" goal="设计输入整理 → 依赖预检查（硬关口）">
 ...
 - 条件：发现 ≥ 2 条会影响后续实现路线的事实分支、约束分支或风险分支
  - 只记录“路线影响线索”，不得在探索阶段（discovery）要求用户选择技术路线或产品路线。
  - 在 `discovery-brief.md` 写清每条分支的事实来源、影响的产品定义问题、可能影响的方案阶段判断、风险和仍需确认的问题。
 ...
 - 等待 `dependency-impact.md` 产出并经 `dependency-validity-reviewer` 通过（PASS）；reviewer verdict 必须通过 `harness contract record-review` 写入调用方 `discovery-brief.contract.yaml.role_verdicts[]`，不得用执行结果或正文摘要替代。
 - 同步写入 `discovery-brief.contract.yaml.dependency_impact`：
   - `required: true`
   - `artifact: harness-runtime/harness/artifacts/<id>/discovery/dependency-impact.md`
   - `review_role: dependency-validity-reviewer`
   - `claims[]`：每条必须包含 `id`、`claim`、`confidence`、`source_evidence`、`failure_mode`、`validation_action`
 - 只有命中决策门或检查点时才等待用户确认，然后继续第 8 步。
...
<step n="9" goal="discovery 有效性审查">
 - 通过 `@discovery-effectiveness-reviewer` native delegation调用 `discovery-effectiveness-reviewer` subagent（Cursor auto-routes 到对应 agent registry 项），输入：当前任务切片的 `discovery-brief.md` 路径、外部 `contracts/discovery-brief.contract.yaml` 路径、`mission-contract.md` 与 `mission-contract.contract.yaml` 路径、`project-context` 与 `project-knowledge/specs` 基线路径。
 - 审查员（reviewer）仅 Read/Grep/Glob；不写简报正文，结论（verdict）必须通过 `harness contract record-review --artifact <contract> --role discovery-effectiveness-reviewer ...` 写入 `contracts/discovery-brief.contract.yaml` 的 `control_contract.role_verdicts[]`。`contract add-verdict` 只用于导入已存在的完整 role_verdict manifest，不作为实时审查默认入口。
 - 无轮次放行（`rounds_used` 只记录修复历史、永不放行，每轮以等同严格度复审）；保持（HOLD）时主流程把 `blocking_gaps` 转回第 2-7 步对应段，让 discovery-analyst 补。卡死时（同一缺口在 analyst 重写后仍以相同根因连续 HOLD 且无实质进展，按缺口本质判断、不是"轮次到点"）不得接受现状降级，重新归因后走 AskUserQuestion 人工裁决（决策候选**不含"接受现状降级"**：强制保持（HOLD）回框定 / 改任务范围 / 升级 BLOCKED）。
 - 条件：审查员结论（reviewer verdict）= 通过（PASS）
  - 继续第 10 步用户确认。
 - 条件：审查员结论 = 保持（HOLD）
  - 按 `blocking_gaps` 回到对应步骤补充，回第 8 步重写简报，再回本步骤复审（无轮次上限，直到 reviewer 在等同严格度下 PASS 或卡死后按上面归因升级）。
 - 条件：审查员结论 = 阻断（BLOCKED）
...
<step n="11" goal="探索阶段退出门">
 - 调 `harness gate transition --stage discovery --mission <id> --mission-slice harness-runtime/harness/work-graph/mission-slices/<id>.yaml --artifact harness-runtime/harness/artifacts/<id>/discovery/discovery-brief.md --contract-artifact harness-runtime/harness/stages/<id>/contracts/discovery-brief.contract.yaml --ai-interpretation "<本次 discovery 关键结论一段话>" --json` 跑阶段退出门（stage exit gate）并在 PASS 时由 CLI 串联推进 Work Graph 与刷新下一 Mission Slice，类型化输出供下游消费。
 - 条件：`gate transition.status = PASS`
  - 阶段完成、Gate 报告、Work Graph operation、下一 Mission Slice 已由 `gate transition` 串联写入；不得再单独补调 `mission stage complete` / `gate advance` / `board select` 造成状态漂移。
 - 条件：`gate transition.status = FAIL`
...
| `` |  | 第 9 步有效性审查卡死（同一缺口在 analyst 重写后仍以相同根因连续保持（HOLD）且无实质进展，按缺口本质判断、非轮次到点；轮次永不构成放行理由）：重新归因后调 AskUserQuestion 走人工裁决（候选**不含"接受现状降级"**：强制保持（HOLD）回框定 / 改任务范围 / 升级 BLOCKED）；裁决后用 `harness approval append --type checkpoint --status approved/rejected` 记录。 |
| `` |  | 第 11 步 `gate transition` 失败（FAIL）：按 `failed_step` / 关口报告（gate report）中的失败列表回到对应步骤重做；不得直接调 `harness mission stage complete` / `gate advance` / `board select` 跳过关口。 |
