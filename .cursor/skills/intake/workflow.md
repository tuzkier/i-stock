...
按缺陷性质分流，不得一律由编排者 inline 修补任务契约：
  ...
  - **实质缺陷回产出者重做**：当保持（HOLD）/ 阻断（BLOCKED）涉及推理链断裂（`reasoning_chain_open`）、目标污染（交付物 / 验收条件 / 工作图标题混入非真实任务目标语料）、用户故事产品上下文缺失（`story` 缺用户 / 问题 / 场景 / 成功指标）、或治理判定断链（治理风险结论与 `autonomy_level` / 检查点 / 风险维度之间缺可追溯依据）这类完备 / 自洽缺陷时，回第 2 阶段重新 `通过 `@mission-framing-expert` native delegation调用 `mission-framing-expert` subagent（Cursor auto-routes 到对应 agent registry 项）` 重做 `intent-framing.yaml`（任务信封须携带本轮 `role_verdict` 的对应 findings 与缺陷分类），再依次经第 3 / 4 阶段重建治理判定与任务契约（Mission Contract），不得由编排者直接改写 `mission-contract.md` 绕过产出者。
