---
name: skill-router
description: '在每轮对话开始时自动触发——决定当前任务应该调用哪些技能。当你犹豫是否需要某个技能、想跳过流程直接动手、或觉得任务太简单不需要技能时必须使用。当用户要求立即把某件事实现 / 落地 / 改掉（执行意图，措辞如「立即执行」「按 XX 文档或需求落地」只是例子）时同样必须先走本技能——「要求马上做」不豁免路由。'
---
...
digraph skill_router {
 ...
 check_brainstorm [label="需求是想法/方向且还没想透？\n或用户说先脑暴/先讨论想透" shape=diamond];
 brainstorm [label="调用 brainstorm\n接入前想透要什么"];
 ...
 check_new -> check_brainstorm [label="是"];
 check_new -> check_signal [label="否"];
 check_brainstorm -> brainstorm [label="是，先想透"];
 check_brainstorm -> intake [label="否，已清晰+执行确认"];
 brainstorm -> intake [label="收敛后交棒" style=dashed];
...
## 新任务的 brainstorm 前置分叉

新任务 / 新需求命中后，进 `intake` 之前先过一个 brainstorm 判断点：

- 需求是**想法 / 方向 / 痛点且"具体要做成什么样"还没想透**，或用户显式说"先脑暴 / 先讨论想透 / 帮我把需求想清楚 / 先别急着开干" → 调用 `brainstorm`，在接入前把"要什么"想透；它收敛后再交棒 `intake`。
- 需求已是**清晰外部结果且带执行确认**（"帮我实现 X / 就这么做"且 X 明确） → 跳过 brainstorm，直接 `intake`。
- 拿不准时由 `brainstorm` 自己在 step-0 用一句话向用户确认"先想透还是直接开始"，不在路由层硬判。

`brainstorm` 是**可选前置**：不建 Mission / 不签契约 / 不写控制面，只反复打磨脑暴活文档。它**不替代** `intake`，收敛物只作为 `intake` 的输入。
...
| "用户是在要求我马上把这件事做出来 / 落地" | 「要求立即实现」正是执行类任务的信号，不是跳过路由的授权。先说清本次路由判断，任务类型仍为新任务/继续/缺陷，落地路径由路由决定。 |
...
4. **工具技能按需**（query-api-docs、ui-ux-pro-max、graphify-*、harness-cli）— 随时可调用，不依赖阶段
...
| 触发信号 | 技能 |
|---------|-------|
| 把 Harness 安装 / 初始化到项目 / "装好了接下来怎么弄" / 续装修复 / 更新或迁移 Harness 框架 / 加 adapter（INSTALL.md scaffold 后的交棒点） | `harness-setup` |
...
| 项目分析 / 代码库分析 / 架构理解 / 代码怎么工作 / 调用链 / 执行流 / 陌生代码库 / "先看下项目" | `graphify-exploring` |
| 影响分析 / blast radius / 会不会影响 / 谁依赖它 / 改 X 会不会炸 / 安全改动评估 | `graphify-impact-analysis` |
| Graphify 索引 / 重新分析仓库 / index stale / 生成 wiki / 查看 Graphify 状态 | `graphify-cli` |
...
| 当前 Mission Slice 声明 `control_plane.stage=interaction` 且 `harness config snapshot` 的 `prototype.delivery_mode=interactive_prototype`（默认），或 `lane_action.skill=interaction`（**每个 mission 默认进入 interaction，要不要原型由该技能 step-0 阶段内判定，不再用 check-ui-trigger 的 UIC 结论做路由门控**） | `interaction` |
...
| `verify` 中验收场景 / 条件失败且根因未知 | `bug-fix` | `bug-fix` | 验证标注失败证据，程序化重跑，AI 定位根因 |
| `verify` 中验收场景 / 条件失败且根因已确认 | `execute` + `bug-fix` | `bug-fix` | 修复必须引用验证 evidence、复现命令和 root cause |
| 用户要求代码质量评估、架构/正确性/安全审查 | `quality-control` | `quality-control` | 程序化命令证据 + 审查员 finding + evidence classification |
| 用户要求阶段性验收、验收证据充分性 | `quality-control` | `quality-control` | 程序化检查验收锚点 / evidence / 阻塞 reason，AI 判断证据充分性 |
...
| interaction / 原型阶段产出 manifest 后的原型 trace 规约、产物级越界检查 | `project-lint` | `project-lint` | 原型阶段当场卡口，不拖到 verify |
...
<HARD-GATE>
在既有项目中，只要进入探索的"当前现状/现有代码/架构理解"分析，若 `graphify-exploring` 技能可用，就必须把它作为探索的辅助技能；不能只用 grep/文件列表替代。Graphify 不可用（未安装、索引缺失或过期）时，必须在 discovery 阶段 evidence carrier 的 `degradations[]` 中结构化记录降级原因和补救动作，不允许只在 markdown 末尾 prose 描述。
</HARD-GATE>
