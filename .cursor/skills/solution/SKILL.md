---
name: solution
description: '当产品定义阶段已完成、需要做方案路线选择 / 关键决策 / 风险边界设计时使用。仅处理 Mission Slice control_plane.stage=solution；不处理 tech-design 与 interaction。用户说"做方案 / 路线选型 / 决策 / 选哪条路 / tradeoff"时触发。'
---
...
`control_plane.stage=solution` 专属 skill。把产品定义包里的"做什么"转化为可决策的"怎么做"路线，产出 `solution.md` + `contracts/solution.contract.yaml`，由 `solution-effectiveness-reviewer` 审查。

## 何时使用

- Mission Slice `control_plane.stage=solution`
- 产品定义阶段已完成、需要做方案路线选择 / 关键决策 / 风险边界设计
...
- 产品定义阶段还未完成 → 先 `prd` skill

## 设计原则

- **目标驱动**：先围绕任务目标、现有架构、风险和长期维护完成方案；只有存在真实分歧路线时，才做方案选择
- **反 demo / 反最小改动**：不得把"先做 demo / 改动最小"作为正式路径
- **可追溯**：每个 decision 必须 traces_to upstream 验收场景 / 系统责任 / 质量与运行约束 / Mission Contract ID
...
| 读产品定义包 + project-context + Mission Contract | inputs |
...
| 系统操作覆盖与自洽校验 | 选定路线是否保留 `SUC-xx-FLOW-xx` 流步骤和 `SUC-xx-OP-xx` 的触发、目标系统操作、对象 / 状态迁移 / 规则和可观察结果。 | 在 `solution.md` 写出系统操作覆盖与自洽校验；方案只能决定承载方式和风险处理，不能重新定义系统行为。 |
...
<exit>
  ...
  - `gate-pass`: harness gate transition --stage solution 返回 status=pass
</exit>
...
<step id="step-1" n="1" goal="solution-architect 调度 + 起草路线和关键决策">
 通过 `@solution-architect` native delegation调用 `solution-architect` subagent（Cursor auto-routes 到对应 agent registry 项）
	 - Task Envelope：任务目标 / 输入路径（`product/product-definition.md`、`product/use-case-model.md`、`product/acceptance-scenarios.md`、`product/product-evidence.md`、`product/product-domain-model.md`、`interaction.md`、`interaction-spec/use-case-realization.md`、`interaction-spec/surface-model.md`、`interaction-spec/interaction-contract.md`、可视化清单如存在、任务契约、project-context、discovery-brief、相关规格）/ 输出路径 `harness-runtime/harness/artifacts/<mission-id>/solution/solution.md` / write_scope（仅 `solution.md`）/ 完成条件。输入摘要必须显式列出 `SUC-xx-FLOW-xx` 流步骤和 `SUC-xx-OP-xx` 系统操作。
...
<step id="step-2" n="2" goal="contract.yaml 初始化 + execution_result 写入">
 ...
 - 调用 `harness contract add-execution-result --artifact harness-runtime/harness/stages/<mission-id>/contracts/solution.contract.yaml --result <solution-architect-execution-result.yaml> --json`，把 solution-architect 返回的 execution_result 写入 contract.yaml。
...
<step id="step-4" n="4" goal="审查循环（无轮次放行）">
 - 循环：无轮次放行（producer-fixable 缺口不设通过上限，轮次只记录修复历史）；退出条件：本轮 solution-effectiveness-reviewer 在等同严格度下返回 PASS
   - Round start：
     通过 `@solution-effectiveness-reviewer` native delegation调用 `solution-effectiveness-reviewer` subagent（Cursor auto-routes 到对应 agent registry 项）
     - brief：`solution.md` + `solution.contract.yaml` + 产品定义包 + 交互 / 原型产物（如有）+ mission-contract.md + project-context 摘要 + 证据图（Evidence Graph）中的下游验证要求 + RUP 方案能力检查点（材料是否足以决策、上游约定是否落到方案、方法执行记录是否完整、路线是否成立、风险是否有处理方式、技术分析是否能继续展开）。
     - 每轮审查返回后调用 `harness contract record-review --artifact harness-runtime/harness/stages/<mission-id>/contracts/solution.contract.yaml --role solution-effectiveness-reviewer --verdict <PASS|PASS_WITH_RISK|HOLD|BLOCKED> --subagent-id <dispatch-id> --model <resolved-model> --review-basis harness-runtime/harness/stages/<mission-id>/solution.md --summary <review-summary> --json`，由 CLI 维护 `role_verdicts` 与审查轮次。
   - 分支：审查结论
 ...
 - 条件：agent_engineering.enabled=true
   ...
   - 循环：无轮次放行（producer-fixable 缺口不设通过上限）；退出条件：agent-capability-reviewer 在等同严格度下返回 PASS
     ...
     - 分支：审查结论
       ...
       - 情况：PASS
         - 调用 `harness contract record-review --artifact harness-runtime/harness/stages/<mission-id>/contracts/solution.contract.yaml --role agent-capability-reviewer --verdict PASS --subagent-id <dispatch-id> --model <resolved-model> --review-basis harness-runtime/harness/stages/<mission-id>/solution.md --summary <review-summary> --json` 后退出循环。
</step>
...
<step id="step-6" n="6" goal="Artifact Gate 自检">
 ...
 - 调用 `harness gate transition --stage solution --mission <mission-id> --mission-slice harness-runtime/harness/work-graph/mission-slices/<mission-id>.yaml --artifact solution.md --contract-artifact contracts/solution.contract.yaml --json`；status != PASS 时按返回 failed_step / failed_checks 修复后重跑。
</step>

<step id="step-7" n="7" goal="Stage 完成 + Work Graph 输出">
 - 第 6 步 `harness gate transition` PASS 后，阶段完成、Gate 报告、Work Graph operation、下一 Mission Slice 已由 CLI 串联写入；不得再单独调用 `mission stage complete` / `gate advance` / `board select`。
 - 当前泳道动作产物 `solution.md` 必须写入 lane_action.output_artifact 对应的 artifact store 路径，并在 contract YAML 的 `work_graph_artifact.artifact_refs[]` 中引用同一 `artifact_id/path`。
...
| `gate-fail` | step-6 harness gate transition FAIL | 按 failed_step / failed_checks 分类修复后重跑 gate transition；连续 3 次 FAIL 时升级 BLOCKED。 |
