...
LEGACY_STAGE_ARTIFACT_MAP = {
    ...
    "technical-analysis/tech-design.md": "tech-design.md",
    "interaction/interaction.md": "interaction.md",
    "product/product-definition.md": "product/product-definition.md",
    "product/product-evidence.md": "product-evidence.md",
    "product/product-domain-model.md": "product-domain-model.md",
    "product/business-object-analysis.md": "business-object-analysis.md",
    "product/use-case-model.md": "use-case-model.md",
    "product/acceptance-scenarios.md": "acceptance-scenarios.md",
    "product/scope-strategy.md": "scope-strategy.md",
}


def artifact_ref_candidates(path_value: str) -> list[str]:
    candidates = [path_value]
    normalized = path_value[2:] if path_value.startswith("./") else path_value
    prefix = "harness-runtime/harness/artifacts/"
    if normalized.startswith(prefix):
        parts = normalized.split("/")
        if len(parts) >= 6:
            mission_id = parts[3]
            suffix = "/".join(parts[4:])
            legacy_suffix = LEGACY_STAGE_ARTIFACT_MAP.get(suffix)
            if legacy_suffix:
                candidates.append(f"harness-runtime/harness/stages/{mission_id}/{legacy_suffix}")
    return list(dict.fromkeys(candidates))


def first_existing_artifact_ref(root: Path, path_value: str) -> Path | None:
    for candidate in artifact_ref_candidates(path_value):
        path = root / candidate
        if path.exists():
            return path
    return None
...
def copy_supplementary_artifacts(root: Path, artifact: dict[str, Any], findings: list[Finding], existence_root: Path | None = None) -> list[dict[str, str]]:
    # Sources are read from ``existence_root`` (real tree under staged apply);
    # canonical copies are written under ``root`` (staging) for atomic commit-back.
    existence_root = existence_root or root
    ...
    for index, spec in enumerate(specs, start=1):
        ...
        if source_glob:
            ...
            strip_root = existence_root / strip_prefix if strip_prefix else None
            if strip_root is not None:
                strip_rel = safe_rel_path(strip_prefix, findings, "strip_source_prefix")
                if strip_rel is None:
                    continue
                strip_root = existence_root / strip_rel
            matches = sorted(path for path in existence_root.glob(str(glob_rel)) if path.is_file())
                ...
                copied.append(
                    {
                        ...
                        "source_stage_artifact": str(source.relative_to(existence_root)),
        ...
        if source_rel is None or canonical_rel is None:
            continue
        source = existence_root / source_rel
...
def promote_artifact(root: Path, operation: dict[str, Any], nodes: dict[str, dict[str, Any]], paths: dict[str, Path], findings: list[Finding], existence_root: Path | None = None) -> None:
    # ``existence_root`` = the real repository root used to validate read-only
    # artifact-ref / source existence. Under staged apply, ``root`` is the
    # transaction staging snapshot (which only copies work-graph / config /
    # stages / missions), so artifact_refs pointing at mission artifacts, the
    # prototype project or traces would spuriously fail existence checks.
    # Resolve their existence against the real tree; node-YAML mutations and
    # canonical copies still write under ``root`` (staging) so the transaction
    # stays atomic and commit-back only touches staged roots.
    existence_root = existence_root or root
    ...
    if isinstance(artifact_refs, list) and artifact_refs:
        ...
        for item in artifact_refs:
            if not isinstance(item, dict):
                findings.append(Finding("FAIL", "invalid_work_graph_artifact", "artifact_refs entries must be objects"))
                continue
            path_value = str(item.get("path") or "")
            if not path_value:
                findings.append(Finding("FAIL", "invalid_work_graph_artifact", "artifact_refs entry missing path"))
                continue
            resolved_path = first_existing_artifact_ref(existence_root, path_value)
            if resolved_path is None:
                findings.append(Finding("FAIL", "artifact_ref_missing", f"Artifact ref path not found: {path_value}"))
                continue
            normalized_refs.append(
                {
                    "artifact_id": str(item.get("artifact_id") or ""),
                    "role": str(item.get("role") or ""),
                    "path": str(resolved_path.relative_to(existence_root)),
        ...
        node["artifact"] = {
            "node_id": node_id,
            "artifact_set_id": str(artifact.get("artifact_set_id") or ""),
            "artifact_version": str(artifact.get("artifact_version") or ""),
            "promoted_by_mission": str(artifact.get("promoted_by_mission") or ""),
    ...
    if any(item.level == "FAIL" for item in findings):
        return
    source = existence_root / str(artifact["source_stage_artifact"])
    ...
    if source_contract or canonical_contract:
        if not (source_contract and canonical_contract):
            ...
            return
        source_contract_path = existence_root / str(source_contract)
    ...
    supplementary_artifacts = copy_supplementary_artifacts(root, artifact, findings, existence_root)
    if any(item.level == "FAIL" for item in findings):
        return
    current_artifact = node.get("artifact") if isinstance(node.get("artifact"), dict) else {}
    history = current_artifact.get("history") if isinstance(current_artifact.get("history"), list) else []
...
def apply_operation_payload(root: Path, graph_root: Path, operation: dict[str, Any], findings: list[Finding], existence_root: Path | None = None) -> None:
    existence_root = existence_root or root
    ...
    if operation_type == "create_node":
        ...
        for index, child in enumerate(operations, start=1):
            ...
            apply_operation_payload(root, graph_root, child_operation, findings, existence_root)
            if any(item.level == "FAIL" for item in findings):
                return
    else:
    ...
    if sum(1 for item in findings if item.level == "FAIL") == initial_failures:
        ...
        if not any(item.level == "FAIL" for item in load_findings):
            promote_artifact(root, operation, nodes, paths, findings, existence_root)
...
def run_direct(root: Path, operation_path: Path, existence_root: Path | None = None) -> dict:
    existence_root = existence_root or root
    graph_root = resolve_graph_root(root)
    ...
    if not findings:
        apply_operation_payload(root, graph_root, operation, findings, existence_root)
...
def run_staged(
    ...
    *,
    ...
    # Mutations happen in the staging snapshot; read-only artifact-ref / source
    # existence is validated against the real repo root so refs to mission
    # artifacts / prototype / traces (not staged) resolve correctly.
    payload = run_direct(staging_root, staged_operation, existence_root=root)
    payload.update(
        {
            "staged": True,
            "dry_run": dry_run,
