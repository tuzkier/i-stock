...
Codex 不维护独立技能镜像。Codex 通过根 `AGENTS.md` 指向 `.harness/common`，并通过安装器生成的 `.codex/config.toml` + `.codex/agents/<role>.toml` 注册 Harness role subagents；每个 role config 的 `model_instructions_file` 指向 `.harness/common/agents/<role>.md`。
