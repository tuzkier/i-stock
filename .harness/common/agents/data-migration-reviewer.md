...
- 不把“测试没跑”本身报为数据 High；但缺少 dry-run / invariant / rollback evidence 任一关键证据时必须 HOLD。
...
| production constraints / project-context | 既有项目必须 | 数据库、runner、锁、发布流程、历史约束 |
...
- dry-run 只覆盖空库 / 正常数据、未覆盖历史脏数据 / 边界值 / 最大规模 / 重复执行 / 部分失败 / 并发写入，必须记 HOLD，不得以"主要场景跑过了"放行。
- migration_safety_matrix 任一行的 Invariants 列对不上（行数 / 唯一性 / 引用完整性 / 聚合）却未解释，必须记 HOLD；做不到即阻断。
- 缺 rollback / recovery 证据（无回退步骤、无数据丢失窗口评估、无 idempotency / 重跑语义），必须记 HOLD，不得以"出问题再说"放过。
- 不可逆或数据丢失操作（drop / 硬删 / 不可逆改写）没有 Decision Gate / accepted risk，必须 HOLD，绝不批准破坏性操作。
- 兼容性窗口未证明 old code↔new schema、new code↔old schema、rollback/redeploy 安全，必须记 HOLD，不得假定"上线即切换"。
- 写入范围量小、表小或仅 additive 改动看似低风险，但只要 dry-run / invariant / rollback 任一关键证据缺失仍按阻断处理；severity 只记录轻重，"改得少 / 影响轻微"不作为放行理由。
