...
- 先读 Parent task 边界、Atomic Task、关联 `SUC-xx-OP-xx`（或明确“不涉及系统操作”的理由）、authorized_paths、prohibited_paths、stop_if、required_evidence。
...
2. **读取上游合同**：读取 Atomic Task、关联 `SUC-xx-OP-xx`、tech-design、interaction-spec、`frontend-changeset.md`、`prototype-as-frontend.contract.yaml`、API contract 和项目约定。没有足够输入时返回 `NEEDS_CONTEXT`。
   - **若该 mission 有 interaction 原型产物（behavior-graph.yaml / surface-model.md）时**：interaction-spec / behavior-graph 不是“参考的上游合同”，而是**实现基准 SSOT**。页面、状态枚举、流程顺序、界面边界（surface create/modify/extend/retire）、以及加载 / 空态 / 错误 / 权限 / 键盘焦点等结局态，必须忠实复现 behavior-graph 对应 `PS-<surf>-<state>` 与 `SUC-xx-FLOW-xx.<state>`，**不得从验收场景自行推导界面长相、自由重设计界面**。验收场景 / 条件只用于补全断言，不覆盖 behavior-graph 对界面的规定。
3. **建立用户可见断言**：把验收场景 / 条件转成页面、组件、状态、URL、文案、ARIA、DOM 状态或 API-backed state 的可观察断言。有 behavior-graph 时，关键 E2E / 交互断言必须绑定到 behavior-graph 的 `step` / `edge`（含 `e2e_obligation` 的 testid），逐项对照 `page_state` 核对实现的页面 / 状态 / 流程。
   - **对照矩阵产出规程（核对有固定分母）**：核对不是自由叙述"已对照"。以 behavior-graph 中本次涉及 surface 的 `PS-<surf>-<state>` 全集为**分母**，逐条产出一行：`page_state（PS-<surf>-<state>）| 实现位置（route/component/状态分支）| 断言 testid（绑定的 e2e_obligation 或 component 断言）| 结果（覆盖 / 缺失 / 超出）`。判定：缺失（分母有、实现无）→ 返回 `BLOCKED` 或走 Decision Gate 并登记 `prototype_coverage_exemptions`，不得静默漏；超出（实现里出现分母外的状态 / 可达操作 / 转移）→ 按界面漂移走 Decision Gate 登记，不得自行重设计；覆盖 → 记下证据锚点。这张对照矩阵就是 Required Evidence 里"原型遵从证据 / 核对记录"的固定形态。
...
- 从 `e2e_locator_obligations`、验收场景、risk 和 bug history 推导 Playwright / component / unit tests。
...
- 当前 Atomic Task 缺少系统操作追溯，且没有明确“不涉及系统操作”的理由。
...
- 该 mission 有 interaction 原型产物，但实现需要偏离 behavior-graph 对应 `page_state` / 流程 / 界面边界（页面、状态枚举、流程顺序、加载 / 空 / 错误 / 权限 / 键盘焦点结局态）——必须停下走 Decision Gate 并登记 `prototype_coverage_exemptions`，不得自由发挥或静默改写界面。
...
- **原型遵从证据（如该 mission 有 interaction 原型产物时）**：**固定形态为「对照矩阵」**——以本次涉及 surface 的 `PS-<surf>-<state>` 全集为分母，逐条记 `page_state | 实现位置 | 断言 testid | 结果(覆盖/缺失/超出)`；实现的页面 / 状态 / 流程逐项对照 behavior-graph `page_state`（`PS-<surf>-<state>`、`SUC-xx-FLOW-xx.<state>`）；关键 E2E / 交互断言绑定到 behavior-graph 的 `step` / `edge`（含 `e2e_obligation` 的 testid）；缺失 → `BLOCKED` / Decision Gate，超出 → 漂移走 Decision Gate，如对原型决策有偏离必须有对应的 Decision Gate 记录或 `prototype_coverage_exemptions` 登记，禁止静默漂移。
