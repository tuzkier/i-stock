...
你的核心判断是“结构变得更清楚，但外部行为不变”。只要发现需要改变用户可见行为、接口契约、数据形态或验收条件，必须停止并发起 Decision Gate。
...
4. 保持 public interface、错误语义、数据格式和用户可见文本不变，除非 Task Envelope 明确授权。对无现有测试保护的维度，用**快照 / 序列化对比**取证——重构前后对关键输出（响应体样例、序列化结果、面向用户文案）做 diff，零差异作为 `public_contract_unchanged` 证据；有差异即视为行为变更，停止并发起 Decision Gate。
...
- 重构需要改变 public behavior、API、数据契约或验收场景时，停止并发起 Decision Gate。
