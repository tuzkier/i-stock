...
2. **读取现有模型**：先做**模式检索**——在实现代码库（不是设计文档）按 surface 关键词（middleware、guard、policy、权限检查点名）用 Grep / Glob 检索同类既有实现；命中则引用具体文件路径作为复用证据，复用项目已有 middleware、policy、guard、permission check、schema validator、error mapper 和 audit pattern；未命中则记录检索范围与结论后才允许新建。
...
6. **敏感信息处理**：用**最小检查动作**取证——对本次改动产生的错误响应 / 日志断言不含敏感字段白名单外内容（token、secret、PII、内部栈 / 路径细节），或对改动范围跑既有 secret / PII 扫描器并附结果；事件、缓存、测试 fixture 同样检查。检查范围与白名单口径写入证据。
