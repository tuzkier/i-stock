...
- **Cursor**：安装器把 `.harness/common/agents/<role>.md` 渲染为 `.cursor/agents/<role>.md` 时，必须把 `adapters.cursor.roles.<role>.candidates[0]` 或对应 default candidates[0] 写入 frontmatter `model`；没有候选时才写 `model: inherit`。
