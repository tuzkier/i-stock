#!/usr/bin/env python3
"""Stage-4 design+interaction PreToolUse hook: block harness gate/contract
advance when interaction.contract.yaml pending_reviewer_recheck=true.
...
def main() -> int:
    ...
    if _pending(_contract_path(cwd, mission_id)):
        print(
            "HarnessV2 design+interaction hook BLOCKED: interaction.contract.yaml "
            "has pending_reviewer_recheck=true. Re-run interaction-reviewer "
            "before advancing.",
            file=sys.stderr,
        )
        return 2
