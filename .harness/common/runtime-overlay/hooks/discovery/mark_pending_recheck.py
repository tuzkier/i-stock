#!/usr/bin/env python3
"""discovery PostToolUse hook: set discovery-brief.contract.yaml
pending_reviewer_recheck=true after any discovery-brief.md edit.

把 discovery-effectiveness-reviewer 的"修复后强制重审"从 prose 升级为物理门：
改了 brief 就必须重跑 reviewer 才能 advance。与 prd / solution 的
effectiveness_review 嵌套口径一致（非 intake 顶层口径）。字段不存在时用 setdefault
注入——discovery-brief 契约 schema 继承 common.yaml 的 additionalProperties:true，
注入不破坏 `harness contract check`。
...
_BRIEF_PATH_RE = re.compile(
    r"harness-runtime/harness/stages/([^/]+)/discovery-brief\.md$"
)


def _extract_mission_from_path(path: str) -> str | None:
    match = _BRIEF_PATH_RE.search(path.replace("\\", "/"))
