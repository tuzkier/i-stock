#!/usr/bin/env python3
"""verify M3.1 PostToolUse hook: mirror acceptance_trace patch mutations to the
evidence graph (acceptance scenario → command_evidence and acceptance scenario → result_evidence edges).
...
typed acceptance-command-result three-chain in:
  harness-runtime/harness/stages/<mission>/traces/evidence_graph.json

Format:
  {
    "nodes": [
      {"id": "<acceptance_id>",  "type": "ac"},
      {"id": "<cmd_id>", "type": "command_evidence"},
      {"id": "<res_id>", "type": "result_evidence"}
    ],
    "edges": [
      {"from": "<acceptance_id>",  "to": "<cmd_id>", "rel": "commands"},
      {"from": "<acceptance_id>",  "to": "<res_id>", "rel": "results"}
    ]
  }
...
def _build_graph(doc: dict) -> dict:
    ...
    for ac in acceptance_traces:
        if not isinstance(ac, dict):
            continue
        acceptance_id = ac.get("id") or ac.get("acceptance_id")
        if not acceptance_id:
            continue
        add_node(acceptance_id, "ac")
