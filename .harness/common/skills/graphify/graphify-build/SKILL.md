---
name: graphify-build
description: "构建 / 重建项目知识图谱（graphify-out/）。当 harness graphify status 返回 available=false 或 fresh=false、需要为既有项目建立或刷新代码+文档知识图谱时使用。代码走本地 AST，文档/Markdown/PDF/图片由当前会话派发子 Agent 语义提取——宿主会话即模型，不需要任何 LLM API Key。"
---

<!--
  Harness 收编自 graphify 官方构建技能（vendored）。
  来源：graphify 0.8.18 `graphify install` 写出的 SKILL.md（trigger /graphify）。
  作用：让"建图能力"随 Harness 一起安装——装完 Harness 即可免 Key 建图，无需在项目里再跑 graphify install。
  免 Key 原理：文档语义提取由宿主会话派发子 Agent 完成（Part B），graphify 不读 ANTHROPIC_API_KEY 等环境变量。
  CLI 子命令（status/query/hook 等）见 graphify-cli 技能；本技能只负责"建图/重建"。
  ⚠️ 同步：升级 graphify CLI 后需重新 vendored 本文件（`graphify install --project` 取新版 SKILL.md 覆盖正文），
     避免与新版 python 抽取 API / fragment schema 漂移。
-->
...
**Large corpora (install-time, super-large projects):** when harness-setup orchestrates this build, it computes a partition/priority/wave plan with `partition_plan.py` (`.harness/common/skills/harness-setup/scripts/partition_plan.py`) over the uncached non-code file list and dispatches **per-module** subagents (one module's batch each → better cross-file semantic locality) **wave by wave** (respecting the parallel limit), highest-file-count modules first. The Part C merge stays a mechanical Python dedup-by-id, so it needs no hierarchical reduce — only the distillation in harness-setup Step 3 does. The flat `ceil(N/22)` path above remains correct for small/medium corpora and standalone `/graphify --update`.
