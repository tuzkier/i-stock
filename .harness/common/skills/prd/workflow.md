...
<step id="step-1" n="1" goal="专业子专家按依赖图产出产品定义输入（含枢纽审查闸）">
 ...
 <dispatch role="business-domain-modeler" mode="spawn" />
 ...
 **Barrier B-审查（枢纽审查闸：业务对象分析）**——按 `core.md`「step 级枢纽审查」执行，先于 `use-case-modeler` 消费：
 <dispatch role="product-definition-reviewer" mode="spawn" />
 ...
 <dispatch role="use-case-modeler" mode="spawn" />
 ...
 **Barrier C-审查（枢纽审查闸：用例模型 + 系统行为描述）**——按 `core.md`「step 级枢纽审查」执行，先于 `acceptance-scenario-designer` / `product-scope-strategist` 消费：
 ...
 <dispatch role="acceptance-scenario-designer" mode="spawn" />
 ...
 <dispatch role="product-scope-strategist" mode="spawn" />
...
<step id="step-2" n="2" goal="资深产品专家综合产品定义包">
 <dispatch role="senior-product-expert" mode="spawn" />
