 ...
 **附加规则（刚完成阶段为 `prd` 且 `spec.enabled=true` 时）：**
 ...
 从 `harness config snapshot --json` 返回值读取 `spec.enabled`。若为 true，除 product/product-definition.md 本身外，还必须至少存在一个 `harness-runtime/harness/artifacts/<mission-id>/product/specs/<capability>/spec.md` 文件，且符合上表差量规格的最小结构要求。否则 FAIL: `missing_delta_spec`。
 ...
 **各文档最小结构要求：**
 ...
 | 文档 | 必须包含 |
 |------|---------|
 | `mission-contract.md` | objective、scope_in、scope_out、acceptance_scenarios、autonomy_level（执行治理级别） |
 | `discovery-brief.md` | 问题空间摘要、用户角色画像、现有方案分析、关键发现、产品定义 输入建议 |
 | product/product-definition.md | 业务定义、成功信号、产品范围、业务 / 系统用例、系统责任与可观察行为、质量与运行约束、追溯矩阵 |
 ...
 | 差量规格（`harness-runtime/harness/artifacts/<id>/product/specs/<capability>/spec.md`，仅 `spec.enabled=true`） | 头部任务/能力/Baseline；至少一个 `## ADDED/MODIFIED/REMOVED Requirements` 段非空；每个 Requirement 至少一个 `#### Scenario`（含 WHEN/THEN） |
...
<step n="2" goal="上游有效性检查（Upstream Validity）">
 ...
 | 检查项 | 可操作判据 |
 |--------|------|
 | AC 引用一致 | 取产出文档（及其 contract）声明的 `traces_to` / 引用 AC 集合，与 mission-contract 的 `acceptance_scenarios[].id` 集合做差集；产出引用了不存在的 AC（差集非空）即 FAIL `ac_reference_mismatch`。只核对引用是否落在已声明 AC 内，不要求全链覆盖。 |
 | 技术决策一致 | 逐字段比对产出文档的技术选型与上游已选定项：solution 阶段比 `solution.md` 的「所选路线」字段，tech-design 阶段比 `tech-design.md` 的选定模块 / 接口 / 数据决策字段；产出在同一字段给出与上游选定项不同的选型且无对应 Decision Gate 记录即 FAIL `tech_decision_mismatch`。 |
 | 范围一致 | 把产出文档实际覆盖的能力 / 改动项映射到 mission-contract 的 `scope_in`；存在落在 `scope_in` 之外（或命中 `scope_out`）的覆盖项即 FAIL `scope_overflow`，须回 course-correction 或 Decision Gate，不得自行扩范围。 |
 | 不是模板空壳 | 与 step-3 的 checker 占位符校验**同口径**：文档主体命中残留占位符（`{{...}}` / `<...>`），或 step-1 最小结构要求的章节存在空段 / 内容长度低于阈值，即 FAIL `artifact_placeholder_stub`。本项不引入第二套判据——能交给程序化 `harness contract check` 判的占位符以 checker 结论为准，此处只在 checker 未覆盖的语义维度补判，避免双标准。 |
 ...
 **需要控制契约的 v1 产物：**
 ...
 | 差量规格 | `harness-runtime/harness/artifacts/<mission-id>/product/specs/<capability>/spec.md` | `harness-runtime/harness/stages/<mission-id>/contracts/delta-spec.contract.yaml` |
 ...
 | execution-brief / 拆解 | `harness-runtime/harness/artifacts/<mission-id>/breakdown/execution-brief.md` | `harness-runtime/harness/stages/<mission-id>/contracts/execution-brief.contract.yaml` |
...
- **interaction 阶段专属跳过守卫**：interaction 现在**每个 mission 默认进入**（不再由 PRD UIC 前置门控），要不要原型由阶段内 step-0 的原型必要性判断决定。因此 interaction 以「无 output_artifact」跳过时，**必须存在确定记录** `harness-runtime/harness/artifacts/<mission-id>/interaction/prototype-necessity.json`（`needed=false` + `reason` + `decided_by`）作为合法跳过凭证；缺该记录的「interaction 无产物」视为**漏做 interaction**（FAIL `interaction_skip_without_determination`），不得当合法跳过放行。`needed=true` 时按下文 interaction 产物 + 角色完整校验。
...
<step n="5" goal="实现质量检查（质量就绪度）">
 ...
 | 测试通过 | 复用 verify 口径：通过**已存在的** `harness verify run-tests`（或 `verify/scripts/collect_command_evidence.py`）生成本次 fresh command evidence，exit code ≠ 0 或 evidence 中 failures > 0 即 FAIL `tests_failed`。判定必须引用该证据产物路径（命令、exit code、统计），不接受"测试已通过"之类自述；陈旧 / 与当前 diff 不匹配的证据等同未运行。 | FAIL |
 ...
 - **project_lint 是每次 gate run 自动强制的硬卡**：`harness gate run` 内部恒会调用控制面报告 checker，checker 自己读取 `harness.yaml` 的 `project_lint`，当 `enabled=true` 且 `require_for_code_change=true` 且当前 diff 含代码改动时：
   - 缺少 project_lint 报告 → FAIL `missing_required_control_report`（先跑 `harness lint project --mission <id>` 再过 gate）；
   - 报告的 `changed_files` 未覆盖当前代码改动（陈旧报告）→ FAIL `stale_control_report`（对最新 diff 重跑 project-lint）。
   - 因此任何改了代码的阶段（execute / bug-fix 及其后续 code-review / verify 等）在 gate run 时都会被卡，不依赖本步骤被显式触发，也不依赖 `--required-control`。
   - 已知边界：freshness 用 `git diff --name-only HEAD`（与 project-lint 同口径），已 commit 但未重新 lint 的改动不在覆盖范围内。

 - **原型阶段同样硬卡**：当 `enabled=true` 且 `require_for_prototype=true` 且 mission 存在 `visual-interaction-manifest.json`（interaction / 原型阶段）时：
   - 缺少 project_lint 报告 → FAIL `missing_required_control_report`（先跑 `harness lint project --mission <id>` 再过 gate）；
   - 报告的 `prototype_manifest_mtime` 早于当前 manifest（陈旧）→ FAIL `stale_control_report`（manifest 变化后重跑 project-lint）。
   - 因此原型阶段的越界 / 不合规（原型 trace 脊柱缺口、受约束产物违规）在 gate run 时当场被卡，不必等到 verify。
...
<step n="8" goal="Work Graph 推进（Gate PASS 后）">
 ...
 - 若当前阶段是 interaction，Gate advance 必须把 `interaction-spec/`、`visual-interaction/` manifest / evidence、长期原型工程引用和用户原型确认记录作为 interaction 产物包的一部分绑定到 Work Graph；否则 execute / e2e-review 不得把这些资产视为已接受依据。下游 AI 以 `interaction-spec/` 为实现合同，长期原型工程主入口是唯一默认人类确认入口；缺少用户确认时不得推进。
 ...
 - 若当前阶段是 execute，contract check 必须确认当前 Mission Slice 没有遗漏同一 `execution_batch_id` / `execution_brief_artifact` 下仍处于当前 execute lane/stage 的 sibling TASK nodes；Gate advance 对多 primary TASK nodes 必须生成 batch `advance_stage`，不得只推进第一个 Parent task 对应的 TASK node。
