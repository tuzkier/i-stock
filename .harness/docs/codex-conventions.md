...
`Codex` 在已安装目标项目中的主入口是**目标项目根 `AGENTS.md`**。该文件的模板源是 `package/adapters/codex/AGENTS.md`；HarnessV2 源码仓库根 `AGENTS.md` 只服务维护本仓库本身。HarnessV2 同时安装 `.codex/config.toml` 和 `.codex/agents/<role>.toml`，用于把 Harness role packages 注册为 Codex subagent types；`.codex/rules/*.rules` 只在项目需要命令执行策略时另行添加。
...
| `.codex/config.toml` | 是 | 注册 Harness role subagents |
| `.codex/agents/*.toml` | 是 | 每个 role 的 Codex config layer，`model_instructions_file` 指向 `.harness/common/agents/<role>.md` |
| `.harness/common/skills/*` | 否 | 作为共享工作流正文手动读取 |
| `.harness/common/agents/*` | 否 | 作为 Harness 子 Agent 角色定义，由 `.codex/agents/<role>.toml` 作为 model instructions 加载 |
...
| 角色化子 Agent | 预定义 Agent 文件 + 任务项 | `.codex/config.toml` 注册的 Codex agent type + workflow 明确声明“调用 `<name>` 子 Agent” |
...
Codex 主 Agent 应按 workflow 声明调用对应子 Agent；安装器生成的 `.codex/config.toml` 会把每个 Harness role 注册为 Codex agent type，并让该 role 的 config layer 通过 `model_instructions_file` 加载 `.harness/common/agents/<name>.md`。除非用户在当前对话中明确禁用子 Agent，否则不得以“缺少授权”为由跳过 Harness workflow 要求的子 Agent 调用；若当前运行环境无法调用指定子 Agent，阶段必须停在 Gate 并报告 BLOCKED，不得由主 Agent 自审自批。

## Agent 调用映射

Codex 当前通过 `.codex/config.toml` 的 `agents.<name>.config_file` 注册 Harness 角色。HarnessV2 在 workflow 中只声明要调用的子 Agent 名称，不声明底层工具名。
...
- Codex 主 Agent 必须确认 `<name>` 已在 `.codex/config.toml` 注册；该 role config 必须通过 `model_instructions_file` 指向 `.harness/common/agents/<name>.md`。
...
- 能按子 Agent 名称解析 `.codex/config.toml` 的 agent registry，并把 `.harness/common/agents/<name>.md` 作为该子 Agent 的 model instructions。
...
- 已支持：安装 `.codex/config.toml` + `.codex/agents/*.toml`，把 Harness role packages 注册为 Codex subagent types
...
5. 如需 sub-agent，先确认 `.codex/config.toml` 已注册对应 role；role instructions 由 `.codex/agents/<role>.toml` 的 `model_instructions_file` 加载
