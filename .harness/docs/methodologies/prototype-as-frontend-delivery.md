...
`interaction` stage 的最终呈现物从"HTML 变体 + preview + manifest"换成"可运行前端工程 patch + MSW + shared types draft"。两种路线的前置设计过程保持一致：先基于 PRD / Domain Model / 验收场景产出 `interaction-spec/`（Surface Interaction Spec），再分别生成 HTML 原型或前端工程 patch。

**关键修正（v2）**：interaction stage 不背"生产级完整代码"的包袱。前端工作要按"每个阶段证明什么"切到不同阶段：

- **interaction**：先证明**界面合同成立**——`interaction-spec/` 对齐 PRD / Domain Model / 验收场景；再证明**效果**——用户在浏览器走查所有 path（happy / 错误 / 空态 / 权限）通过；shared types 是 draft；MSW 够走查
...
intake → discovery → prd → interaction → solution → tech-design → breakdown → execute → code-review → verify → delivery → finishing-branch
                            ...
                            │                          │                      │           │           │
                  ┌─────────┴──────────┐   ┌──────────┴────────┐  ┌──────────┴──┐ ┌──────┴────┐ ┌────┴────┐
                  │ spec + 效果：       │   │ shared types       │  │ e2e 实现 +  │ │ 完整 a11y │ │ 真后端  │
                  │ surface spec +      │   │ freeze + e2e test  │  │ 测试加固 +  │ │ + lint + │ │ e2e +   │
                  │ patch + MSW +       │   │ plan 设计          │  │ 切真 API + │ │ coverage │ │ 场景验证 │
                  │ 用户走查（无 e2e）  │   │                    │  │ 联调 + 修复 │ │ audit    │ │         │
                  └────────────────────┘   └────────────────────┘  └────────────┘ └──────────┘ └─────────┘
```

**`interaction` 阶段的输入** —— PRD 全套：`product-definition.md` / `product-domain-model.md` / `prd.contract.yaml` / `mission-contract.md` / `api-contract-draft.md`。前端工程 patch 不是只消费 `interaction-spec/`，而是联合消费 PRD / Domain Model / 验收场景 / `interaction-spec/`；其中 `interaction-spec/` 承担界面级决策和 traceability 的 canonical handoff。

**`interaction` 阶段的产出**：

| 路径 | 用途 | 备注 |
|---|---|---|
| `harness-runtime/harness/stages/<mission-id>/interaction.md` | interaction 阶段总览、PRD 回流检查、用户反馈同步记录 | 与 `interaction-spec/` 一起作为下游入口 |
| `harness-runtime/harness/stages/<mission-id>/interaction-spec/` | Surface Interaction Spec；由 PRD / Domain Model / 验收场景派生的界面交互合同 | 两种 delivery mode 共用；前端 patch 必须对齐它 |
...
| `harness-runtime/harness/stages/<mission-id>/frontend-changeset.md` | 本次前端工程 patch 记录（实现文件、types、MSW、locator、与 `interaction-spec/` 的差异处理） | 不是前置交互合同 |
...
- `verify`：**Playwright e2e against 真后端 PASS** + 验收场景验证（MSW 不参与本阶段）
- `delivery` / `finishing-branch`：照常

**用户验收 checkpoint：** 用户在浏览器走查 PRD 列出的全部用户 path（happy / 错误 / 空态 / 权限），通过 scenario 切换器观察各分支，PASS。

**反馈同步规则：** 用户对 HTML 原型或前端工程提出变更时，先分类再更新资料。界面布局、控件选择、文案、状态反馈、信息层级、locator 等变更必须先回写 `interaction-spec/` 和 `interaction.md`，再重建 HTML 或前端 patch；新增 / 删除用户路径、验收场景、业务规则、权限、领域状态或范围时必须回流 PRD / Domain Model / Decision Gate；只改最终呈现而不更新 spec / PRD 的状态不得进入后续 stage。
...
- `interaction.md` + `interaction-spec/`：由 PRD / Domain Model / 验收场景派生的界面交互合同；后续用户反馈必须同步回写
...
- 项目级前端工程根目录：优先写在 `project-knowledge/engineering/policies/stage-rules.yaml` 的 `interaction.frontend_project_root`；也可由项目说明显式声明。全新项目如果缺失，interaction 阶段必须先询问用户并回写项目级说明，不得由 skill 或专家角色默认成某个目录。

**真正的差异：**

| | 单独原型 | 原型即前端交付（v2） |
|---|---|---|
| 阶段内中间产物形态 | `interaction-spec/` + 可视化资产 | `interaction-spec/` + 前端实现证据（TS draft 类型 + mock + frontend-changeset） |
...
| 评审方式 | reviewer 看 PRD + spec + variant | reviewer 看 PRD + spec + 代码 + changeset，并在浏览器点真交互（不评 e2e / a11y 评分 / coverage） |
...
项目级 config（`project-knowledge/engineering/policies/stage-rules.yaml`）固化以上栈和前端工程根目录，所有 mission 沿用。如某 mission 需要换栈或改目录，由 `solution` stage 发起 Decision Gate 决策，结论回写项目 config。
...
export const workspaceHandlers = [
  ...
  http.post("/api/workspaces", async ({ request }) => {
    ...
    if (!body.repositoryUrl?.startsWith("git@")) {
      return HttpResponse.json({ error: "ERR-SSH-ONLY: SSH only" }, { status: 422 });
    }
...
| supplementary_artifacts | `interaction-spec/**` + `visual-interaction/variants/**` + `preview/**` + `visual-interaction-manifest.json` | `interaction-spec/**` + `frontend-changeset.md` + `<frontend-project>/mocks/` + `<frontend-project>/lib/types/` + user-walkthrough |
...
- 前端工程能 `pnpm dev` 跑起来
- `interaction-spec/` 已通过 spec-check / ux-quality-check / interaction-reviewer，且无 spec_vs_prd_drift
- MSW 覆盖**所有 user path**（happy / 错误 / 空态 / 权限 / 边界），scenario 切换器齐全
- PRD 和 `interaction-spec/` 列出的所有用户 path 在浏览器可点可看
- 用户在浏览器实际跑通全部 path（不是看图想象）
- `lib/types/**` **draft** 覆盖 PRD `api-contract-draft.md` 全部 endpoint（不要求 freeze）
- `frontend-changeset.md` 完整：引用 `interaction-spec/`，记录 spec_conformance；每条改动有 `spec_ref` / `operation` / `baseline_ref`（非 create 时）/ `traces_to` 验收场景或用例 / `domain_refs` / `e2e_locator_obligations`
- 无 frontend_vs_spec_drift；用户反馈已同步到 `interaction-spec/` 或 PRD / Decision Gate
...
| 单独原型 | 原型即前端交付 | 职责变化 |
|---|---|---|
| `interaction-designer` | `interaction-designer` + `frontend-prototype-engineer` | 先由 interaction-designer 产出 renderer-independent 的 `interaction-spec/`，再由 frontend-prototype-engineer 实现 surface patch + MSW + shared types draft + frontend-changeset |
| `interaction-reviewer` | `interaction-reviewer` + `frontend-reviewer`（四维 verdict） | 先评 `interaction-spec/` 是否对齐 PRD / Domain Model / 验收场景；再评 frontend patch 的 domain_coverage / surface_baseline / path_completeness / walkthrough_ready；**不评 e2e / a11y 评分 / coverage / lint**（那些是 code-review 的事） |
...
control_contract:
  ...
  produced_artifacts:
    - path: harness-runtime/harness/stages/<mission-id>/interaction.md
    - path: harness-runtime/harness/stages/<mission-id>/interaction-spec/
    - path: <frontend-project>/
    - path: <frontend-project>/lib/types/
    - path: <frontend-project>/mocks/
    - path: harness-runtime/harness/stages/<mission-id>/frontend-changeset.md
  obligations:
    - id: OBL-INTERACTION-001
      type: frontend_implementation
      required_evidence:
        - interaction_reviewer_verdict
        - reviewer_verdict (frontend-reviewer)
        - interaction_spec_complete
        - pnpm_dev_starts
        - frontend_changeset_complete
        - msw_path_coverage
        - api_contract_draft_present
        - e2e_locator_ready
        - user_browser_walkthrough_passed (用户在浏览器实际跑通全部 P0 场景)
      blocking: true
  role_policy:
    required_execution_roles: [interaction-designer, frontend-prototype-engineer]
    required_review_roles: [interaction-reviewer, frontend-reviewer]
