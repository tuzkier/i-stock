...
| Element | Definition | Source | Used By | Failure If Missing |
|---|---|---|---|---|
| Existing Capability | 当前系统、流程或知识库已有能力 | codebase、project context、knowledge、Graphify | PRD、Solution、Tech Design | 重复设计或破坏既有行为 |
...
| Evidence Source | 支撑发现的具体来源 | files、commands、docs、Graphify、API docs | PRD、Stage Gate | 结论不可追溯 |
...
| Business Rule | 约束对象行为、状态或权限的业务规则 | business reference、domain analysis | Scenario、验收条件、Tests | 验收条件变成泛泛描述 |
| Scenario / 验收条件 | 用户场景、Given-When-Then 和可观察验收结果 | Mission、business rules、scope strategy | Interaction、Breakdown、Verify | 验收不可执行 |
| Scope Decision | In / Out / Later / Decision Needed | Mission、risk、dependency | Solution、Breakdown、Delivery | 未授权范围进入实现 |
| Validation Signal | 证明验收条件达成所需的证据类型 | Scenario design | Verify、Code Review | 测试通过但需求无法验收 |
...
| User Journey | 用户从触发到完成目标的路径 | Scenario、验收条件、domain action | Prototype、E2E | 只画页面，不证明路径 |
...
| Verification Strategy | 证明设计正确的测试和证据路径 | 验收场景、risk、module design | Breakdown、Code Review、Verify | 任务完成无法被证明 |
...
| Element | Definition | Source | Used By | Failure If Missing |
|---|---|---|---|---|
| Parent Task | 可独立交付的纵切任务 | tech-design、验收场景 | Execute、Stage Gate | 任务切片只按文件或层拆 |
...
| Element | Definition | Source | Used By | Failure If Missing |
|---|---|---|---|---|
| Finding | 可定位、可复现、可修复的问题 | diff、tests、design、验收场景 | Execute fix loop | 审查变成泛泛建议 |
| Severity | High / Med / Low 与是否 HOLD | exploitability、验收影响、risk | Stage Gate | 阻断项被放行 |
...
| Element | Definition | Source | Used By | Failure If Missing |
|---|---|---|---|---|
| Acceptance Under Verification | 正在验证的验收项 | product-definition、mission contract | Delivery | 验证不对应需求 |
| Expected Result | 按验收场景应观察到的结果 | PRD、spec | verification report | 只跑命令没有预期 |
...
| Result Evidence | 可观察结果证据 | screenshots、API output、data state | Delivery | 测试通过不等于验收通过 |
...
| Acceptance Item | 用户可逐条验收的项目 | 验收场景、verification | User | 交付包不可自验 |
