...
每个会进入 execute 的 task / Atomic Task 必须包含以下字段。不能只写“补测试”“按 TDD”或“覆盖验收场景”。

| 字段 | 说明 |
|------|------|
| `behavior_under_test` | 本任务要证明的可观察行为，必须追溯到验收场景 / Scenario / Parent task |
...
1. 从验收场景 / Scenario 提取行为模型：输入空间、决策规则、状态迁移、错误路径和不变量。
...
| “实现接口并测试 happy path” | 忽略错误路径和边界 | 按验收场景 / Scenario 写合法、非法、权限、状态、数据边界 |
