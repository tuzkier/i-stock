...
| 决策 | 决策者 | 依据 |
|------|--------|------|
| 本任务项需要哪些测试能力 | Harness 拆解 | 验收场景 / Scenario / risk_level / surfaces |
...
TDD 审查员只能判断测试是否能发现错误实现；它不能把缺工具、缺报告、缺 obligation 判成测试质量问题，也不能用整体 PASS 覆盖某个验收场景的证据缺口。
...
| medium / 普通业务逻辑、API、UI 状态 | test_result + changed-file coverage 或等价 targeted evidence | 无验收场景 / 任务项映射则 FAIL |
...
- 测试命令 PASS，但 Evidence Graph 中验收场景没有 result evidence。
