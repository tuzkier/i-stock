...
| `<permissions>` / `<invariant>` / `<condition>` / `<criterion>` / `<deny>` / `<allow>` / `<input>` / `<artifact>` / `<failure>` / `<recovery>` / `<stage_transition>` / `<next>` | 默认改用 Markdown 表格或列表；除非已有解析器明确消费这些标签并在 lint allowlist 中登记 |

Workflow 不再承载 `<permissions>` 权限岛。运行时权限、hook 和阶段覆盖由 `.harness/common/runtime-overlay/`、role policy、CLI gate 或 adapter 原生配置维护；workflow 只描述行为约束和调用方式，避免同一权限语义散落在 workflow 与 overlay 两处。
