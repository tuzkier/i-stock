...
本次重写的核心变化：PRD 不再只描述“自选 + K 线 + 信号 + 简单提醒”，而是定义一个本地看盘工作台（workbench）。用户可观察的闭环必须覆盖自选列表状态、行情来源健康、默认日常看盘工作台、指标切换、MTS 解释卡、提醒 taxonomy、本地触发历史、布局模式和浏览器重开恢复；诊断 / 验收模式用于展开来源、提醒、恢复和布局细节。
...
| BUC-02 | 打开默认看盘工作台 | 个人投资者、技术分析用户 | 进入标的后直接获得自选、图表、MTS 和状态摘要；诊断 / 验收模式可展开提醒与来源细节 | 用户选择 active 标的 | BO-001, BO-003, BO-004, BO-005, BO-007, BO-008 | ChartLayout `focus/default`、`dense/diagnostic`；PriceSeries `ready/stale/unavailable` | P0 | detail | yes | AC-02, SD-04, SD-05 |
...
2. 工作台进入 `focus/default` 日常看盘布局：左侧自选，中间主图/成交量/副图/MTS，提醒、来源和验收信息以摘要入口呈现。
3. 主图展示 PriceSeries；成交量 pane 常驻；副图 pane 显示默认指标。
4. 用户需要排查时可切换 `dense/diagnostic`，展开提醒、来源、恢复和布局细节；来源健康状态仍是一级可达信息，不只是一句偶发 notice。
...
**Scenario BUC-02-S01 选中标的后进入默认日常看盘工作台**

- **GIVEN** 用户已有 active 自选标的
- **WHEN** 用户选中该标的
- **THEN** 页面展示自选区、图表区、MTS 卡和提醒 / 来源 / 验收摘要入口
- **AND** 图表区包含主图、成交量 pane 和一个副图 pane
- **AND** 诊断 / 验收模式可展开提醒、来源、恢复和布局细节
...
- AC-BUC-02-01：首开 active 标的必须出现默认日常看盘工作台或移动端等价 tab，且诊断 / 验收模式可达。
...
- **WHEN** 系统恢复工作台
- **THEN** 系统回到默认 focus 日常看盘布局或等价可用布局
- **AND** 自选和提醒仍尽可能恢复
