...
**业务目标：** 交付一个本地网页形态的多市场看盘工作台。用户能按市场管理自选并处理代码歧义，打开标的后进入默认日常看盘工作台，查看主图/成交量/副图指标与 MTS 解释卡；提醒规则、来源健康、恢复和布局细节在诊断 / 验收模式中展开，并在同一浏览器中恢复自选、提醒、触发历史和基础布局。
...
| SCN-02 | 技术分析用户 | 打开标的快速看盘 | 单图或指标堆叠不足以支撑判断 | 默认日常看盘工作台，主图、成交量、副图指标切换、OHLC 和 MTS 同时可读；诊断 / 验收模式可展开提醒与来源 |
...
| BUC-02 | 打开默认看盘工作台 | 个人投资者、技术分析用户 | 进入默认日常看盘或移动端等价工作台，诊断 / 验收信息可展开 | BO-001, BO-003, BO-004, BO-005, BO-007, BO-008 | yes | AC-02, FR-02 |
...
| In | 默认日常看盘工作台、主图/成交量/副图切换、OHLC 可读、诊断 / 验收细节可展开 | 用户自然期待先看核心工作台，再按需展开来源、提醒和恢复细节 | SD-04, SD-05 |
...
| 原型 / 交互 | 歧义预览、日常看盘工作台、指标切换、MTS 卡、提醒 taxonomy、来源健康、布局模式 | interaction spec、截图、走查记录 | 每个 P0 BUC 有可走查路径 |
...
当前 Mission Slice 已回到 `interaction` 阶段。本 PRD 是按用户要求基于界面功能参考与原型走查反馈进行的上游回改。后续不得继续直接推进旧 solution；必须先让 interaction / solution 重新消费本 PRD 中的 `PriceSeries`、`MarketDataSource`、`AlertRule taxonomy`、`ChartLayout focus/dense/mobile_tab`、`source health panel` 和 `fixture-first` 约束。
