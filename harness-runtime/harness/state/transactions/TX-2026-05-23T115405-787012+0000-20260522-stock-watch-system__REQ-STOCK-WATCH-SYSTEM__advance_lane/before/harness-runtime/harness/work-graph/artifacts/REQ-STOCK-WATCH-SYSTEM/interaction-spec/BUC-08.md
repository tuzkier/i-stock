...
在桌面端切换 dense 与 focus；在移动端使用 `mobile_tab` 访问自选、图表、提醒和来源，保证不同设备下都能读得清、切得快。
...
```text
布局：[紧凑 dense] [专注 focus] [移动分栏 mobile_tab]
桌面：左自选 / 中图表 / 右提醒与来源
移动： [自选] [图表] [提醒] [来源]
```
...
P1 E2E seed: 在 desktop dense / focus 间切换，断言工作台列宽变化；在 mobile_tab 下断言四个 tab 都可访问。
