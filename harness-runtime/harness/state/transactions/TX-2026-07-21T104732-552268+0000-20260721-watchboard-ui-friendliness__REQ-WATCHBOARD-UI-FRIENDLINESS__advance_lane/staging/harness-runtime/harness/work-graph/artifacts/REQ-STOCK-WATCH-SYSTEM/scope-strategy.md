...
| SD-04 | In | 默认三栏工作台 | 桌面端默认采用“自选 / 图表 / 信号与提醒”三栏工作台，不要求用户先自定义布局才能进入核心路径。 | BO-008；BUC-02/04/05；AC-02 | Open-source UI ref §工作台布局 |
...
| SD-04 | 默认三栏工作台把自选、图表、信号/提醒放进同一观察面，符合扫描效率优先。 | 若要求先配置布局，会破坏首开闭环。 | `docs/open-source-ui-reference.md` §6 | BUC-02/04/05 |
...
- 这是一次 **上游 PRD 回改**。`solution.md` 后续必须重对齐以下新增边界：`输入歧义预览`、`默认三栏工作台`、`MTS 解释卡`、`提醒 taxonomy + 本地触发历史`、`来源健康面板`、`dense/focus/mobile tab`、`fixture-first 验证输入`。
