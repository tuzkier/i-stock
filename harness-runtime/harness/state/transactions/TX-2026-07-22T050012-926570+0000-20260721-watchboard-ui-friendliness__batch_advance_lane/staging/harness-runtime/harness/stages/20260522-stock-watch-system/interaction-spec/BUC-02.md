...
选中 active 标的后，直接进入默认日常看盘工作台：左侧自选，中间图表 / MTS / 状态摘要；提醒、来源、恢复和布局细节可在诊断 / 验收模式展开，不必先配置布局。

## Entry / Exit

- Entry: 用户点击自选行的“看盘”，或选中一个 active 标的。
- Exit: 工作台展示默认日常看盘布局，或在数据不足时进入可解释降级；诊断 / 验收模式保持可达。

## ASCII Wireframe

```text
┌ Watchlist ─────┐ ┌──── WorkbenchShell / DailyStatusDock ─────────────┐
│ AAPL  active   │ │ AAPL / US / AAPL                                  │
│ 0700.HK active │ │ [ChartSurface 主图] [Volume] [IndicatorPanel]     │
│ 600519 archived│ │ [MTS card] [提醒摘要] [来源摘要] [验收摘要]       │
└────────────────┘ └────────────────────────────────────────────────────┘
诊断 / 验收 dense：展开 Source / Alerts / Restore / Layout 细节。
```
...
- 默认日常看盘布局必须始终可回退；若恢复粒度不明确，则回到可用默认布局。
...
P0 E2E seed: 选中 `symbol-row-aapl` 后断言默认日常看盘工作台可见，图表与来源状态共享同一标的上下文，且诊断 / 验收入口可达。
...
- 日常看盘工作台是任务主路径，不是装饰性排版；诊断 / 验收是同一信息架构的展开层。
