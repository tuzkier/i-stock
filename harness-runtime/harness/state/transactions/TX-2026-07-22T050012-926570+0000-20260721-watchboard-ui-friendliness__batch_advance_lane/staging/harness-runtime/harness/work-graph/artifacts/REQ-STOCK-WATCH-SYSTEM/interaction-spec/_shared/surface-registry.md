...
| SURF-WORKBENCH | `WorkbenchShell`, `ChartSurface`, `IndicatorPanel`, `DailyStatusDock` | `modify_surface` | `visual-interaction/prototype/index.html` | BUC-02, BUC-03, BUC-04, BUC-08 | 选中 active 标的后进入 / mobile `chart` tab | 日常看盘、主图、成交量、副图、OHLC、提醒 / 来源 / 验收摘要入口 | `workbench-shell` |
...
- 桌面 focus（日常看盘）：左侧 Watchlist，中间 Workbench + DailyStatusDock，Alerts / Source / Restore 以摘要入口进入。
- 桌面 dense（诊断 / 验收）：左侧 Watchlist，中间 Workbench，右侧 Alerts / Source / Restore 细节展开。
