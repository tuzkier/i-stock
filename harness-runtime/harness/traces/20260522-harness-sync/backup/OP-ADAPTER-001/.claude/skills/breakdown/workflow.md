...
<step id="step-1" n="1" goal="专业角色调度">
 ...
 通过 `Task(subagent_type="delivery-slicer", prompt=<Task Envelope>)` 工具调用 `delivery-slicer` subagent
 通过 `Task(subagent_type="test-planning-expert", prompt=<Task Envelope>)` 工具调用 `test-planning-expert` subagent
...
<step id="step-6" n="6" goal="一次性写盘 + reviewer 派发">
 ...
 通过 `Task(subagent_type="execution-plan-effectiveness-reviewer", prompt=<Task Envelope>)` 工具调用 `execution-plan-effectiveness-reviewer` subagent
