...
<step id="step-2" n="2" goal="审查-修复循环">
 - 循环：id=reviewer-loop；max_rounds=3；退出条件：本轮汇总后无 High 级别问题
  ...
  通过 `Task(subagent_type="correctness-reviewer", prompt=<Task Envelope>)` 工具调用 `correctness-reviewer` subagent
  通过 `Task(subagent_type="tdd-reviewer", prompt=<Task Envelope>)` 工具调用 `tdd-reviewer` subagent
