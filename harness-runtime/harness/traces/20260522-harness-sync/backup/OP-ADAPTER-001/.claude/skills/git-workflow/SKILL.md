...
| `close` | retrospective PASS 后 | 决定 mission branch 去向（合并/PR/保留/丢弃） |
...
docs(prd): finalize requirements [M001]
...
| 技能 / Rule | 关系 |
|-------------|------|
| `autonomy-loop` | 会话恢复调 recover；阶段开始前调 start-stage；Stage Gate PASS 后调 commit-artifact；retrospective PASS 后调 close |
