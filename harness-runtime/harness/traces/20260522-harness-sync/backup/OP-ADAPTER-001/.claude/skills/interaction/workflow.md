...
通过 `Task(subagent_type="interaction-designer", prompt=<Task Envelope>)` 工具调用 `interaction-designer` subagent
...
通过 `Task(subagent_type="interaction-reviewer", prompt=<Task Envelope>)` 工具调用 `interaction-reviewer` subagent
