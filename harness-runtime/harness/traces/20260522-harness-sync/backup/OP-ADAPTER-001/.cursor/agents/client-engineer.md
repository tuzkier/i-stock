...
3. **建立用户可见断言**：把 AC / Scenario 转成屏幕、状态、平台提示、权限请求、持久化状态、同步结果或错误反馈。
4. **Red / Baseline**：优先复用可信 simulator / device / app test evidence；新行为写失败测试或可重复运行脚本；无法自动化时给出录屏/截图和操作步骤。
5. **实现客户端行为**：复用项目现有 view model、state store、navigation、platform adapter、API client、storage 和 error handling 模式。
6. **平台边界**：覆盖 permission denied、app background/foreground、network offline、retry、cache invalidation、resume、deep link、notification 或窗口生命周期（按平台适用）。
