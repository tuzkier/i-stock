...
- 不动 `.harness/common/**` 框架正文（那是 `/harness-update` 的事）
- 不动 runtime 数据
...
- 目标项目**尚未**有该 adapter 的根入口文件（如 claude → `CLAUDE.md`、windsurf → `.windsurfrules`）。若已存在，停下并提示走 `/harness-update` 或人工 diff
...
- 在 `.harness/docs/install-guide.md` 中确认新 adapter 已被识别（version metadata 含 adapter 列表的话同步更新）
