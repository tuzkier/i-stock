...
**Goal:** 从 Work Graph Board 选择可推进工作对象，并把一次推进包装成 Mission Slice。
...
<step n="2" goal="选择可推进 node 并生成 Mission Slice">
 - 按 `work_graph.selection_strategy` 选择 node：先按当前任务上下文相关性，再按 node priority，再按 lane action priority / status / updated_at / node_id 等稳定顺序。
...
<step n="4" goal="Stage Gate 后应用推进">
 ...
 - Mission Slice 若需要非线性图操作，必须携带 `graph_operation` payload；Gate 后支持 `advance_lane`、多 primary node batch advance、`split_node`、`merge_nodes`、`block_node`、`defer_node`、`supersede_node` 和 `batch`。
