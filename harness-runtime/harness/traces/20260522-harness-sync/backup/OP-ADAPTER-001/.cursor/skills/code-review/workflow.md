...
<step id="step-2" n="2" goal="审查-修复循环">
 - 循环：id=reviewer-loop；max_rounds=3；退出条件：本轮汇总后无 High 级别问题
  ...
  通过 `@correctness-reviewer` native delegation调用 `correctness-reviewer` subagent（Cursor auto-routes 到对应 agent registry 项）
  通过 `@tdd-reviewer` native delegation调用 `tdd-reviewer` subagent（Cursor auto-routes 到对应 agent registry 项）
