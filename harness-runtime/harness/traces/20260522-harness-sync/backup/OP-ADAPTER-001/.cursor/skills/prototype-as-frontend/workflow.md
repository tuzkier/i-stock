...
通过 `@frontend-prototype-engineer` native delegation调用 `frontend-prototype-engineer` subagent（Cursor auto-routes 到对应 agent registry 项）
...
通过 `@frontend-reviewer` native delegation调用 `frontend-reviewer` subagent（Cursor auto-routes 到对应 agent registry 项）
