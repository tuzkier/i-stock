...
| verify / stage-gate | 验收结果证据、AC trace、contract 与 artifact 一致性 |
...
|------|----------|-------------------|
| Necessity | 这个 AC / user journey 是否真的需要浏览器级证明，还是已有低层证据足够 | 关键用户旅程没有 E2E，也没有有效替代 |
...
3. 建立 `AC / Scenario / Task -> user journey -> E2E test -> oracle -> evidence` 矩阵。
