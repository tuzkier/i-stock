...
| 安装 / 更新 / 迁移指南 | `INSTALL.md`；安装后为 `.harness/docs/install-guide.md` |
...
|------|------|
| 元技能 | `skill-router` |
| 规划 | `intake`, `discovery`, `prd`, `design`, `breakdown` |
...
| 工具集成 | `query-api-docs`, `ui-ux-pro-max`, `visual-interaction-design`, `cognee-knowledge` |
| GitNexus | `gitnexus-exploring`, `gitnexus-impact-analysis`, `gitnexus-cli`, `gitnexus-debugging`, `gitnexus-refactoring`, `gitnexus-pr-review`, `gitnexus-guide` |

每个技能的正文位于 `.harness/common/skills/<skill>/SKILL.md`；若存在 `workflow.md`，执行阶段以 `workflow.md` 为细节依据。GitNexus 技能位于 `.harness/common/skills/gitnexus/<skill>/`。
...
| 命令 | 用途 | 资产类别 |
|---|---|---|
| `harness-update` | 用新模板刷新 `.harness/common/**` 与 `.harness/docs/**` | A 类（框架） |
| `harness-migrate` | 完整模板迁移：保留 runtime 数据，按权限化 operation 切换到新模板 | A+B+C+D+E |
...
子 Agent role 源定义位于 `.harness/common/agents/<name>.md`。安装到目标项目时，支持原生子 Agent registry 的 adapter 必须把这些 role 物化到对应目录，例如 `.harness/common/agents/<name>.md`、`.harness/common/agents/<name>.md`、`.opencode/agents/<name>.md`。workflow 声明调用 `<name>` 时，优先使用 adapter 原生命名子 Agent；只有当前 runtime 没有原生 registry 时，才由 workflow 按可用调度能力注入 role 正文并附加 Task Envelope。
...
| `acceptance-scenario-designer` | prd 执行（验收场景 / GWT / AC） |
...
| 阶段文档 | `harness-runtime/harness/stages/` | 各阶段产出文档 |
| 差量规格 | `harness-runtime/harness/stages/<id>/specs/<capability>/spec.md` | 本次任务行为变更 |
...
| 用户验收结果 | `harness-runtime/harness/stages/<id>/acceptance-result.md` | 面向人的交付入口 |
| 交付包 | `harness-runtime/harness/stages/<id>/delivery-package.md` | 内部交付归档 |
| 长期记忆 | `harness-runtime/harness/memory/` | 项目级知识沉淀 |
...
| `harness-runtime/harness/stages/<id>/specs/<capability>/spec.md` | 本次任务差量规格 |

规格层流程：探索识别受影响能力；PRD 产出差量规格；拆解引用 Scenario；执行以差量规格为边界；code-review 逐条核查 Scenario；任务收尾把确认后的长期行为契约提炼固化到 project-knowledge。
