...
**质量标准**：objective 必须一句话讲清楚；scope_out 每条必须附理由；acceptance_criteria 必须可验证
...
**核心回答**：用户怎么操作、关键状态有哪些、异常怎么处理、领域对象如何映射到界面、哪些结构化原型合同和可视化资产支撑这个判断

**质量标准**：PRD 后默认需要产出，除非任务明确为 API-only / 无界面 / 纯后端 / CLI-only。interaction.md 必须根据产品定义、差量 spec 和 DDD 领域模型说明原型界面、信息架构、用户路径、状态矩阵和 PRD 回流检查；涉及复杂 UI / user journey 时，必须引用按真实系统 surface 组织的 `interaction-spec/` 作为 AI handoff 合同，并引用 `visual-interaction/visual-interaction-manifest.json`、HTML / SVG 变体、preview 或等价截图 / 录屏作为人类评审证据。修改既有 UI 时必须写明 baseline 和 surface changeset，不能按任务堆孤立页面。HTML / 图片不能替代 `interaction-spec/`。
