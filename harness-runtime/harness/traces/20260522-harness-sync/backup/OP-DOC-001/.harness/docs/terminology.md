...
4. 工具和产品名不翻译，例如 HarnessV2、Codex、Cursor、Claude Code、GitNexus、OpenCode。
5. 行业缩写按原文保留，必要时在首次出现时解释，例如 PRD、TDD、SDD、E2E、API、AC。
...
| 验证义务 | Obligation | 某个 AC / 任务项必须被什么证据证明的要求。字段名保留 `obligations`。 |
...
| 原型合同 | Interaction Spec | interaction 阶段给下游 AI 消费的结构化原型实现合同，目录名保留 `interaction-spec/`；`visual-interaction/prototype/index.html` 只作人类确认的可操作原型，不是实现合同。 |
