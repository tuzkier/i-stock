...
def _has_sufficient_evidence(ac_id, ac, result_evidence) -> tuple[bool, str]:
...
def _check_contract_acs(doc: dict) -> list[str]:
    ...
    ac_traces = inner.get("ac_trace") or []
    ...
    for entry in ac_traces:
        ...
        ok, reason = _has_sufficient_evidence(ac_id, entry, result_evidence)
        if not ok:
...
def _build_evidence_graph(doc: dict) -> dict:
    ...
    for ac in inner.get("ac_trace") or []:
