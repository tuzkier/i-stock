...
当用户要求质量评估、Stage Gate 报告 evidence 缺口、验证无法支撑 AC 结论、审查员返回 HOLD，或 retrospective 中出现重复质量缺口时，使用本协议。

## 控制平面绑定

| 平面 | Runtime 职责 |
|------|--------------|
| Intent | 读取任务 AC、范围、NFR、执行治理级别和 Checkpoint |
...
2. 构建追溯矩阵：`AC -> 需求/场景 -> 设计约束 -> Task -> 变更文件 -> 测试/证据 -> Review/Verify 结果`。
...
Hard Gate 必须有具体证据引用。没有证据的 AC pass 无效。工具不可用是 verification 缺口，不是 pass。

## 禁止捷径

- 不得用自述替代命令输出或审查员结果。
- 不检查 AC / requirement / 任务项 / test 链路时，不得判断代码质量。
- 不得把覆盖率百分比当作充分性证明。
