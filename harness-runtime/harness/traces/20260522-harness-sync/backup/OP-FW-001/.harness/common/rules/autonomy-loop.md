...
- `RUNTIME_ROOT/missions/<mission-id>/mission-contract.md`（有活跃任务时）— 目标、范围、AC、治理级别和 Checkpoint 配置；不作为链路调度源
...
| `lane_action.stage=prd` | Mission Contract 的 intent / AC、primary node、已 accepted 的上游 Work Graph artifact、discovery 产物（仅当该 node / slice 引用） |
...
| `lane_action.stage=verify` | 当前 node、verification obligations、命令证据、AC trace、project-lint/e2e/tdd 状态 |
...
| 条件 | 调度的技能 |
|------|-------------|
| 项目没有 `project-context.md`（棕地项目） | `generate-context` |
| 没有活跃任务契约 | `intake` |
...
3. 当 workflow、lane action 或 role policy 返回多个执行角色 / 审查角色时，主 Agent 必须按 barrier 并行调度：同一执行 barrier 的独立 execution roles 并行；执行 barrier 全部返回后进入审查 barrier；同一审查 barrier 的 readonly reviewers 并行。只有存在明确产物依赖、共享写入范围或 workflow 显式声明必须串行时才允许串行，并记录原因。
...
3. 等待审查员返回裁决：
 ...
 - HOLD（仍有阻断性问题）→ 回到 Step 3b 继续修复，再进入 3d（循环，上限 3 次）
 - 循环 3 次仍有阻断性问题 → 发起 Decision Gate，向用户展示遗留问题清单，等待用户拍板
```
