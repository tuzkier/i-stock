...
the AI calls a gitnexus MCP tool (gitnexus_query / gitnexus_context).

Triggered on PostToolUse with tool_name matching mcp__gitnexus__* (or the
canonical names gitnexus_query / gitnexus_context). The hook locates the
...
brownfield path actually exercised gitnexus.
...
# Recognize gitnexus tools regardless of MCP server prefix convention:
# - mcp__gitnexus__query / mcp__gitnexus__context / mcp__gitnexus__repos
# - bare gitnexus_query (legacy)
# - any future variant that contains "gitnexus" in the tool name.
_GITNEXUS_TOOL_TOKENS = ("gitnexus",)


def _is_gitnexus_tool(tool_name: str) -> bool:
    name = (tool_name or "").lower()
    return any(token in name for token in _GITNEXUS_TOOL_TOKENS)
...
def main() -> int:
    ...
    if not _is_gitnexus_tool(tool_name):
        return 0
