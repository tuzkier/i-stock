#!/usr/bin/env python3
"""HarnessV2 intake PreToolUse hook: block `harness gate/contract advance` when
contract has unresolved reviewer recheck.

Triggered on PreToolUse(Bash). Enforces the intake workflow's
`no-skip-recheck-after-fix` HARD-GATE physically: refuses to advance the gate
or contract when `control_contract.pending_reviewer_recheck` is true (the
post-edit hook sets this flag whenever the mission-contract is modified).
...
def _pending_flag(contract_path: Path) -> bool:
    ...
    if not isinstance(contract, dict):
        return False
    return bool(contract.get("pending_reviewer_recheck"))
...
def main() -> int:
    ...
    if mission_id is None:
        # Without an explicit --mission flag we can't locate the contract.
        # Allow rather than block to avoid false positives on aggregate
        # advance commands.
        return 0
