...
WORKFLOW_EXPECTATIONS = {
    ...
    ".harness/common/skills/technical_analysis/workflow.md": {
        "must_reference_path": ["harness alignment check"],
        "must_mention_concept": ["interaction flows/states"],
    },
...
def check_cli_assets(root: Path, results: list[dict[str, str]]) -> None:
    ...
    for sub in (["trace", "log-init"], ["trace", "step-enter"], ["trace", "step-exit"]):
...
def check_verify_template_fields(root: Path, results: list[dict[str, str]]) -> None:
    ...
    Rules (W-verify-*):
      W-verify-workflow: workflow.md has Step 0, failure_paths, check-ac-trace call,
        gate run call, and no bare spawn_agent.
      W-verify-schema: verification_evidence schema has required_evidence_id field.
      W-verify-template: verification-report.contract.yaml template has ac_trace,
    ...
    if workflow_path:
        ...
        workflow_checks = [
            ...
            ("check-ac-trace", "W-verify-workflow-check-ac-trace", "verify workflow calls contract check-ac-trace"),
    ...
    # W-verify-template: ac_trace, command_evidence, result_evidence
    ...
    if template_path:
        ...
        for marker, code, pass_msg in [
            ("ac_trace", "W-verify-template-ac-trace", "verification-report.contract.yaml declares ac_trace"),
...
def check_workflow_authoring_style(root: Path, results: list[dict[str, str]]) -> None:
    ...
    them explicitly; permissions are parsed from Markdown tables by install.py.
    """
    workflow_roots = [
        root / ".harness/common/skills",
        root / ".harness/common/skills",
...
def check_finishing_branch_template_fields(root: Path, results: list[dict[str, str]]) -> None:
    ...
    # W-finishing-mission-close-enum: harness_cli.py must contain new enum values
    cli_path = existing_path(root, ".harness/common/cli/harness_cli.py")
    if cli_path:
        cli_text = cli_path.read_text(encoding="utf-8")
        new_values = {"merged", "pr", "kept", "discarded"}
        missing_values = [v for v in sorted(new_values) if f'"{v}"' not in cli_text and f"'{v}'" not in cli_text]
        if missing_values:
            results.append(finding(
                "FAIL", "W-finishing-mission-close-enum",
                f"harness_cli.py missing new close enum values: {missing_values}",
            ))
        else:
            results.append(finding(
                "PASS", "W-finishing-mission-close-enum",
                "harness_cli.py contains all new close enum values (merged/pr/kept/discarded)",
            ))
    else:
        results.append(finding(
            "FAIL", "W-finishing-mission-close-enum",
            ".harness/common/cli/harness_cli.py not found",
        ))

    # W-finishing-cli-commands: harness_cli.py must register finishing-branch subparser
    if cli_path:
        if "finishing-branch" in cli_path.read_text(encoding="utf-8"):
            results.append(finding(
                "PASS", "W-finishing-cli-commands",
                "harness_cli.py registers finishing-branch subparser",
            ))
        else:
            results.append(finding(
                "FAIL", "W-finishing-cli-commands",
                "harness_cli.py does not register finishing-branch subparser",
            ))
...
def check(root: Path) -> dict[str, Any]:
    ...
    for rel, expected in WORKFLOW_EXPECTATIONS.items():
        ...
        for path_ref in expected["must_reference_path"]:
            if path_ref in text:
                results.append(finding("PASS", "workflow_reference_path", f"{rel} references {path_ref}"))
            else:
