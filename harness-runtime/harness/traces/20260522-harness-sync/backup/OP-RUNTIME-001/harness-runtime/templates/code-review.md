...
> **来源**：code-review 技能 → `harness-runtime/harness/stages/{{mission_id}}/code-review.md`
...
| ID | 严重级别 | 类别 | 摘要 | 状态 | 处理引用 |
|----|----------|------|------|------|----------|
| FND-001 | medium | correctness | {{finding_summary}} | fixed | {{resolution_ref}} |
...
| AC/Scenario/任务项 | 测试追溯 | Red 有效性 | 断言强度 | 充分性 | Fault Detection | 结论 |
|------------------|----------|------------|----------|--------|-----------------|------|
| {{trace_target}} | {{test_refs}} | valid / missing / invalid | strong / weak | adequate / 缺口 | proven / missing | pass / hold |

### TDD Blocking Gaps

| ID | 严重性 | TDD 问题类型 | 关联 AC/Scenario/任务项 | 缺口 | 为什么阻断 | 为什么这是 TDD 问题 | 必须补什么 |
|----|--------|---------------|----------------------|------|------------|--------------------|------------|
...
| AC/Scenario/任务项 | e2e_obligation | E2E 测试追溯 | 用户可观察结果断言 | 数据真实性 | 负向路径 | Realtime/Refresh | 可靠性 | 诊断 Artifact | 结论 |
|------------------|----------------|---------------|--------------------|------------|----------|------------------|--------|---------------|------|
...
| ID | 严重性 | E2E 问题类型 | 关联 AC/Scenario/任务项 | 缺口 | 为什么阻断 | 为什么这是 E2E 问题 | 必须补什么 |
|----|--------|---------------|----------------------|------|------------|--------------------|------------|
