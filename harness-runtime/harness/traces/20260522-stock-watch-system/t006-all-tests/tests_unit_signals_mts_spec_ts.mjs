// tests/unit/signals/mts.spec.ts
...
function mtsReasonRegistry2() {
  return mtsReasonRegistry();
}
...
describe("MTS explanation", () => {
  it("returns a structured technical alert without investment advice language", () => {
    const signal = buildSignal(bars(), sourceHealth("formal"));
    assert.equal(signal.mts.signalType, "technical_alert");
    assert.notEqual(signal.mts.scoreBand, "not_applicable");
    assert.ok(signal.mts.reasonCodes.includes("TREND_ABOVE_EMA"));
    assert.ok(signal.mts.reasonCodes.every((code) => code in mtsReasonRegistry2()));
    assert.match(signal.mts.technicalReminder, /不构成收益承诺/);
    assert.doesNotMatch(signal.mts.displayLabel, /买点|卖点|收益|胜率/);
  });
  it("does not output a valid alert level when data is insufficient", () => {
    const signal = buildSignal(bars(8), sourceHealth("formal"));
    assert.equal(signal.mts.trendState, "data_insufficient");
    assert.equal(signal.mts.mtsScore, null);
    assert.equal(signal.mts.alertLevel, "none");
    assert.deepEqual(signal.mts.reasonCodes, ["DATA_INSUFFICIENT"]);
  });
  it("invalidates MTS when source health is degraded", () => {
    const signal = buildSignal(bars(), sourceHealth("unavailable"));
    assert.equal(signal.mts.trendState, "source_degraded");
    assert.equal(signal.mts.mtsScore, null);
    assert.equal(signal.mts.alertLevel, "none");
    assert.ok(signal.mts.reasonCodes.includes("SOURCE_DEGRADED"));
  });
});
