...
export type NormalizationPreview =
  ...
  | {
      status: "ambiguous" | "invalid" | "duplicate_active";
      rawInput: string;
      selectedMarket: MarketCode;
      message: string;
...
const suffixMarket: Record<string, MarketCode> = {
  HK: "HK",
  SS: "CN",
  SZ: "CN",
  KS: "KR"
...
export function cleanTickerInput(symbol: string) {
  return symbol.trim().toUpperCase().replace(/\s+/g, "");
}

export function normalizeTickerForMarket(symbol: string, market: MarketCode) {
...
function isValidTickerForMarket(clean: string, market: MarketCode) {
...
function explicitMarket(clean: string): MarketCode | null {
  const match = clean.match(/\.([A-Z]{2})$/);
  return match ? suffixMarket[match[1]] ?? null : null;
}

function numericCandidates(clean: string): NormalizationCandidate[] {
