...
test("默认工作台展示主图、成交量、副图、OHLC，并按需进入来源", async ({ page }) => {
  ...
  await expect(page.getByTestId("chart-ohlc")).toContainText("O ");
...
test("数据不足时副图局部降级，不展示伪造读数", async ({ page }) => {
  ...
  await expect(page.getByTestId("mts-reason-list")).toContainText("DATA_INSUFFICIENT");
});
...
test("demo fallback 来源健康和降级语义穿透到图表、信号与提醒区域", async ({ page }) => {
  ...
  await expect(page.getByTestId("signal-degradation-note")).toContainText("demo_fallback");
  await page.getByTestId("detail-tab-source").click();
